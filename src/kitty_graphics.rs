...
mod tests {
    ...
    /// The budgeted, coalescing arm of the incremental encoder: one bounded
    /// transaction per call, re-displaying every cached placement that fits.
    const BUDGET: usize = HEADLESS_GRAPHICS_TRANSACTION_BUDGET;
    ...
    fn budgeted_update(
        ...
        cache.request_placement_replay();
        encode_graphics_update_incremental(cache, placements, &HashSet::new(), Some(budget), true)
    }
    ...
    fn terminal_only_headless_budget_does_not_let_large_image_starve_small_image() {
        ...
        let large_only = budgeted_update(&mut cache, std::slice::from_ref(&large), BUDGET);
        assert!(large_only.bytes.is_empty());
        assert_eq!(cache.oversized.len(), 1);
        assert!(cache.images.is_empty());

        let hidden = budgeted_update(&mut cache, &[], BUDGET);
        assert!(hidden.bytes.is_empty());
        assert_eq!(cache.oversized.len(), 1);

        let with_small = budgeted_update(&mut cache, &[large, small], BUDGET);
        assert!(String::from_utf8_lossy(&with_small.bytes).contains("a=t"));
        assert!(with_small.bytes.len() <= BUDGET);
    ...
    /// A Unicode-placeholder image reaches the incremental encoder as one
    ...
    fn budgeted_image_rows_upload_in_one_transaction() {
        ...
        let upload = budgeted_update(&mut cache, &placements, BUDGET);
    ...
    fn budgeted_image_rows_disappear_in_one_transaction() {
        ...
        let mut cache = HostGraphicsCache::default();
        loop {
            let encoded = budgeted_update(&mut cache, &placements, BUDGET);
            if !encoded.incomplete {
                break;
            }
        }

        let removed = budgeted_update(&mut cache, &[], BUDGET);
    ...
    fn budgeted_image_rows_delete_old_rows_together_before_replacement() {
        ...
        let mut cache = HostGraphicsCache::default();
        loop {
            let encoded = budgeted_update(&mut cache, &old, BUDGET);
            if !encoded.incomplete {
                break;
            }
        }
        ...
        let cleanup = budgeted_update(&mut cache, &replacement, BUDGET);
        ...
        let replaced = budgeted_update(&mut cache, &replacement, BUDGET);
    ...
    fn budgeted_image_rows_redisplay_in_one_transaction() {
        ...
        let replay = budgeted_update(&mut cache, &placements, BUDGET);
    ...
    fn budgeted_pixel_uploads_do_not_coalesce() {
        ...
        let first_pass = budgeted_update(&mut cache, &placements, BUDGET);
        assert!(
        ...
        let second_pass = budgeted_update(&mut cache, &placements, BUDGET);
    ...
    fn budgeted_redisplay_coalescing_respects_budget() {
        ...
        let tight = budgeted_update(&mut cache, &placements, 100);
        ...
        let rest = budgeted_update(&mut cache, &placements, 100);
    ...
    fn budgeted_upload_keeps_other_redisplays_out() {
        ...
        let mut cache = HostGraphicsCache::default();
        loop {
            let encoded = budgeted_update(&mut cache, std::slice::from_ref(&cached), BUDGET);
            if !encoded.incomplete {
                break;
            }
        }
    ...
    fn budgeted_superseded_image_delete_does_not_coalesce() {
        ...
        let mut cache = HostGraphicsCache::default();
        loop {
            let encoded = budgeted_update(&mut cache, &pair(), BUDGET);
            if !encoded.incomplete {
                break;
            }
        }
    ...
    fn budgeted_pane_cleanup_precedes_terminal_image_upload() {
        ...
        let cleanup = budgeted_update(&mut cache, std::slice::from_ref(&terminal), BUDGET);
        assert!(cleanup.incomplete);
        let cleanup = String::from_utf8(cleanup.bytes).unwrap();
        ...
        let upload = budgeted_update(&mut cache, &[terminal], BUDGET);
        assert!(String::from_utf8_lossy(&upload.bytes).contains("a=t"));
    }
    #[test]
    fn oversized_quarantine_stays_bounded_and_clears_with_the_cache() {
