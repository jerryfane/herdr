...
pub struct App {
    ...
    /// The fork's monolithic mode: `herdr --no-session` runs without a shared
    /// server, so it never saves or restores a session and gram is unavailable
...
// Fork-only: `herdr accounts` writes the config file through this helper, shared
// with the CLI so the readiness gate and the writer agree on layout.
mod config_io;
