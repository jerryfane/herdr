/// Marker for "the prompt text never reached the PTY".
///
/// A submission fails in two materially different ways: the text write never
/// landed, so the pane saw nothing and a client may safely resend; or it landed
/// and something later went wrong, where a resend risks a double prompt. The
/// text-phase failure is tagged with this kind so the API can answer
/// `agent_prompt_not_received` instead of a generic failure.
...
/// Fork behaviour, re-applied on upstream's submission queue.
///
/// A prompt is written as text, then Enter after a delay. If the pane changes
/// hands during that delay the Enter must NOT be sent: it would submit into
/// whatever occupies the pane now. `occupant_unchanged` is evaluated once,
/// immediately before the Enter write, and `watch` records which way it went so
/// the API can report submitted vs abandoned instead of guessing.
#[derive(Clone)]
