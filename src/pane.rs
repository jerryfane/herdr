            ...
            PaneRuntimeIo::TestChannel { sender, .. } => {
                ...
                std::thread::spawn(move || {
                    let result = if text.is_empty() {
                        // Both real actors skip the text write entirely when the text
                        // is empty; sending an empty frame here would make the double
                        // channel disagree with the PTY about what reached the pane.
                        Ok(())
                    } else {
