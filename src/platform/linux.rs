...
mod tests {
    ...
    fn finite_clipboard_commands_report_exit_status() {
        // Absolute interpreter on purpose: this test is about reporting a child's
        // exit status, not about PATH resolution, and other modules' tests mutate
        // PATH. Resolving "sh" through PATH made this fail only in a full-suite
        // run, which is the worst kind of failure to debug.
        // Shell BUILTINS only, and an absolute interpreter: this test is about
        // reporting a child's exit status, and other modules' tests mutate PATH.
        // `cat` resolved through a mutated PATH returned 127 and made this fail
        // only in a full-suite run. The read loop drains stdin so the parent's
        // write never sees EPIPE.
        let success = ClipboardCommand {
            program: "/bin/sh",
            args: &["-c", "while read -r _; do :; done; exit 0"],
        };
        let failure = ClipboardCommand {
            program: "/bin/sh",
            args: &["-c", "while read -r _; do :; done; exit 7"],
        };
