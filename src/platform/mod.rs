...
/// One IPv4 address found on a local network interface, with the interface that owns it.
///
/// The interface NAME is carried because the address is what a user scans from a QR code:
/// on a machine with docker bridges or several NICs, `192.168.1.5 (en0)` tells them
/// whether the right one was picked and `192.168.1.5` alone does not.
#[derive(Debug, Clone, PartialEq, Eq)]
