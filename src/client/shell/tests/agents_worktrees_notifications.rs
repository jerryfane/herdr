...
#[test]
fn agent_sidebar_shows_input_waiting_status_only_while_a_prompt_is_pending() {
    let sidebar_text = |input_pending: bool| {
        let mut projected = snapshot();
        projected.agents = vec![ClientShellAgent {
            ...
            state_change_seq: 1,
            input_pending,
            input_prompt_kind: input_pending.then_some(crate::detect::InputPromptKind::Select),
            state_labels: vec![("blocked".into(), "needs input".into())],
            tokens: Vec::new(),
            focused: true,
        }];
        let mut config = Config::default();
        config.ui.sidebar.agents.rows = vec![vec![crate::config::AgentSidebarToken::StateText]];
        let mut state = ClientShellState::new(ClientShellConfig::from_config(&config));
        state.set_snapshot(Box::new(projected));
        state.set_pane_surface(surface());
        let frame = state.compose(106, 30).expect("agent sidebar frame");
    ...
    let pending = sidebar_text(true);
    assert!(
        pending.contains("input waiting · select"),
        "frame: {pending}"
    );
    assert!(
        !pending.contains("needs input"),
        "the pending prompt replaces the lifecycle status: {pending}"
    );

    let settled = sidebar_text(false);
    assert!(!settled.contains("input waiting"), "frame: {settled}");
    assert!(settled.contains("needs input"), "frame: {settled}");
}
