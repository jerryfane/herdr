...
#[test]
fn mobile_switcher_shows_input_waiting_status_only_while_a_prompt_is_pending() {
    let switcher_text = |input_pending: bool| {
        let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
        let mut projected = snapshot();
        projected.agents.push(ClientShellAgent {
            pane_id: "pane_1".into(),
        ...
        state.set_pane_surface(surface());
        state.mode = ClientShellMode::Navigate;
        let switcher = state.compose(44, 30).expect("mobile switcher");
        switcher
            .cells
            .chunks(switcher.width as usize)
            .map(|row| {
    ...
    let pending = switcher_text(true);
    assert!(
        pending.contains("input waiting · select · pi"),
        "switcher: {pending}"
    );

    let settled = switcher_text(false);
    assert!(!settled.contains("input waiting"), "switcher: {settled}");
    assert!(settled.contains("waiting · pi"), "switcher: {settled}");
}
