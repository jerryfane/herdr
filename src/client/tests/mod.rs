...
fn forward_clipboard_uses_local_clipboard_path() {
    // `SSH_CONNECTION` is process-global and read by the remote-detection path
    // several other tests exercise, so take the one env lock and restore it
    // through the guard rather than by hand.
    let _guard = env_lock().lock();
