...
/// Status text shown while an agent-owned input prompt is waiting. This
/// replaces the lifecycle status in the agent sidebar and the mobile switcher.
fn input_waiting_text(kind: Option<crate::detect::InputPromptKind>) -> &'static str {
    use crate::detect::InputPromptKind;
    match kind {
        Some(InputPromptKind::Confirm) => "input waiting · confirm",
        Some(InputPromptKind::Select) => "input waiting · select",
        Some(InputPromptKind::FreeText) => "input waiting · free-text",
        Some(InputPromptKind::Unknown) | None => "input waiting",
    }
}
