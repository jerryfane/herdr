    ...
    fn paste_client_clipboard_image_path(
        ...
        match target {
            ...
            protocol::ClientClipboardImageTarget::Pane(pane_id) => {
                ...
                // A clipboard-image paste is a human write into the pane; the
                // fork routed it through the app input layer, which recorded it.
                ...
                match apply_client_pane_input_events(
                    ...
                    Ok(()) => {
                        self.app.record_pane_composer_write(
                            workspace_index,
                            runtime_pane_id,
    ...
    fn handle_server_event(&mut self, ev: ServerEvent) -> bool {
        ...
        match ev {
            ...
            ServerEvent::ClientInput { client_id, data } => {
                ...
                // The fork attributes a direct terminal-attach write to the human
                // composer so `agent.prompt` can verify submission.
            ...
            ServerEvent::ClientShellPaneInput {
                ...
                let scroll_before = runtime.scroll_metrics();
                // The fork attributes a human keystroke or paste into a pane to
                // the human composer so `agent.prompt` can verify submission and
                // report `ComposerAuthor::Human`. The baseline must be read
                // BEFORE these bytes are written, so the attempt covers this
                // write rather than the pane content that preceded it.
                let composer_baseline = runtime.detection_content_seq();
                let opens_composer_attempt = client_pane_input_commits_text(&events);
                let applied = match apply_client_pane_input_events(runtime, &events) {
                    Ok(()) => true,
                    Err(err) => {
                ...
                if applied {
                    // A human typing continues one attempt instead of starting a
                    // new one per keystroke, so this never forces a new attempt;
                    // a navigation-only batch only refreshes an attempt that
                    // already exists.
                    self.app.record_pane_composer_write(
                        workspace_index,
                        runtime_pane_id,
...
/// Whether a client-shell input batch commits literal text into the pane, which
/// is what may open a human composer attempt. The fork's own `is_text_commit`
/// predicate decides that for keys, so a navigation-only batch (arrows, page
/// keys, chorded keys, releases, mouse) never opens an attempt.
fn client_pane_input_commits_text(events: &[protocol::ClientPaneInputEvent]) -> bool {
    events.iter().any(|event| match event {
        ...
        .is_text_commit(),
        protocol::ClientPaneInputEvent::TextCommit(text)
        | protocol::ClientPaneInputEvent::Paste(text) => !text.is_empty(),
