...
pub fn run_server() -> io::Result<()> {
    ...
    let result = rt.block_on(async {
        // TODO(merge-v0.9.0): the fork's server entry point also set
        // `app.state.local_sound_playback = false`, `app.local_terminal_notifications
        // = false` and `app.local_input_source_switch = false` so the headless
        // process never played sound or emitted notifications itself. Upstream
        // forwards notifications to client shells instead and dropped those
        // fields from `App`; re-apply the suppression here if the merged `App`
        // keeps them.
