...
fn test_headless_server_with_event_hub(event_hub: api::EventHub) -> HeadlessServer {
    ...
    // A process-unique counter, not just a timestamp: two threads reading the clock
    // in the same tick produced the same directory, and the second `bind` then failed
    // with AddrInUse (seen once in 12 full runs). The counter makes the name unique by
    // construction rather than by clock granularity.
...
/// The fork records a Human composer write for every human keystroke or paste
/// into a pane, so `agent.prompt` can verify submission and `agent.get` can name
/// the author. Upstream deleted the app-level input layer that held those call
/// sites, which left the client-shell (TUI) key path with no accounting at all.
/// A navigation-only batch must still not open an attempt.
#[tokio::test]
async fn client_shell_text_input_opens_human_composer_attempt() {
    let mut server = test_headless_server();
    let mut input_rx = install_focused_test_runtime(&mut server, b"prompt>");
    server.app.state.ensure_test_terminals();
    let internal_pane = server.app.state.workspaces[0].tabs[0].root_pane;
    let terminal_id = server.app.state.workspaces[0]
        .terminal_id(internal_pane)
    ...
    terminal.agent_name = Some("planner".into());
    ...
    let key = |code, text: Option<&str>| crate::protocol::ClientPaneInputEvent::Key {
        code,
        modifiers: 0,
        kind: crate::protocol::ClientKeyKind::Press,
        repeat_count: 1,
    ...
    let composer = |server: &mut HeadlessServer| {
        let mut panes = headless_pane_list(server);
        assert_eq!(panes.len(), 1, "single test pane");
    ...
    assert_eq!(
        navigation_only.author, None,
        "navigation must not open a composer attempt"
    );
    assert_eq!(navigation_only.attempt_id, None);

    // Only the first batch changes render state (it promotes the client to
    // foreground); the accounting proof is the composer attempt below.
    server.handle_server_event(ServerEvent::ClientShellPaneInput {
        client_id: 1,
        pane_id: pane_id.clone(),
        events: vec![key(crate::protocol::ClientKeyCode::Char('h'), Some("h"))],
    });
    assert!(!input_rx.recv().await.expect("encoded typed key").is_empty());
    let typed = composer(&mut server);
    assert_eq!(
        typed.author,
        Some(crate::api::schema::ComposerAuthor::Human),
        "a typed client-shell key is a human composer write"
    );
    let attempt_id = typed.attempt_id.expect("typed key opens an attempt");
    ...
    assert_eq!(
        ...
        Some(attempt_id),
        "typing continues one attempt instead of starting one per keystroke"
    );
    shutdown_test_runtimes(&mut server);
}
