...
mod tests {
    ...
    fn unreadable_loose_ref_dir_is_unavailable_not_absent() {
        ...
        // Root ignores directory permissions, so the unreadable-dir condition this
        // test needs cannot be constructed as uid 0 and the read would succeed.
        // Skipping is honest; weakening the assertion would delete the coverage
        // for every non-root developer and for CI.
        if unsafe { libc::geteuid() } == 0 {
            eprintln!(
                "skipping unreadable_loose_ref_dir_is_unavailable_not_absent: running as root, \
                 which bypasses the directory permissions this test depends on"
            );
            return;
        }
