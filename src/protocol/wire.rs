...
pub struct ClientShellAgent {
    ...
    /// Live input-prompt presence for this agent, mirrored from the API pane
    /// state so the client shell can render the "input waiting" status.
    /// Optional on the wire: an older server omits it and an older client
    /// ignores it.
    #[serde(default)]
