...
impl TerminalKey {
    ...
    /// Whether this key commits literal text on a Press: a bare character with
    /// only text-shaping modifiers (empty, or SHIFT for an uppercase letter).
    /// Used by composer-attempt detection to recognize typed input.
