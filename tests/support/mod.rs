...
/// Spawned servers and the runtime directories they own, each recorded against
/// the test thread that created it.
///
/// The owner matters because the panic hook below fires on one thread while
/// every other test in the binary is still running: a registry keyed only by
/// pid cannot tell "this test's leaked server" from "eleven other tests'
/// live servers", and killing the latter turns one real failure into a dozen
/// `EmptyResponse`/`ConnectionReset` reports in innocent tests.
static PID_REGISTRY: LazyLock<Mutex<HashMap<u32, ThreadId>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
static RUNTIME_DIR_REGISTRY: LazyLock<Mutex<HashMap<PathBuf, ThreadId>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
...
pub fn unregister_runtime_dir(path: &Path) {
    runtime_dir_registry_lock().remove(path);
}
...
/// Kills every server this process spawned. Only correct when the process is
/// going away (`atexit`, Ctrl-C), because it does not distinguish owners.
pub fn cleanup_registered_herdr_pids() {
    ...
    let runtime_dirs: HashSet<PathBuf> = {
        let mut runtime_dirs = runtime_dir_registry_lock();
        runtime_dirs.drain().map(|(dir, _)| dir).collect()
    };
...
/// Kills only the servers the panicking test spawned.
///
/// A panicking test never reaches its own `cleanup_spawned_herdr`, so its
/// server would leak; but its siblings on other threads are mid-request, and
/// their servers are not this test's to reap.
fn cleanup_current_thread_herdr_pids() {
    let owner = thread::current().id();

    let pids: Vec<u32> = {
        let mut registry = pid_registry_lock();
        let owned: Vec<u32> = registry
            .iter()
            .filter(|(_, thread_id)| **thread_id == owner)
            .map(|(pid, _)| *pid)
            .collect();
        for pid in &owned {
            registry.remove(pid);
    ...
    let runtime_dirs: HashSet<PathBuf> = {
        let mut registry = runtime_dir_registry_lock();
        let owned: HashSet<PathBuf> = registry
            .iter()
            .filter(|(_, thread_id)| **thread_id == owner)
            .map(|(dir, _)| dir.clone())
            .collect();
...
fn ensure_cleanup_hooks() {
    INIT.call_once(|| {
        ...
        std::panic::set_hook(Box::new(move |panic_info| {
            cleanup_current_thread_herdr_pids();
...
fn pid_registry_lock() -> std::sync::MutexGuard<'static, HashMap<u32, ThreadId>> {
    PID_REGISTRY
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
}

fn runtime_dir_registry_lock() -> std::sync::MutexGuard<'static, HashMap<PathBuf, ThreadId>> {
...
fn registered_runtime_dirs_snapshot() -> HashSet<PathBuf> {
    runtime_dir_registry_lock().keys().cloned().collect()
}
