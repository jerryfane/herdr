...
On the **remote** machine, explicitly set `reverse_coordinator_machine_id = "<coordinator-install-id>"` under `[federation]` (the grant command prints the ID). The granted agent calls `herdr agent federated-list --caller-pane <local-terminal-id>` and `herdr agent federated-prompt <coordinator-terminal-id> "message" --caller-pane <local-terminal-id>`; `HERDR_PANE_ID` can supply the caller ID instead. The list contains coordinator agents and only explicitly permitted peer agents, with the coordinator's observed reachability and status. Prompts to stale/offline targets are refused; successful responses preserve the API's delivery result. The SSH host must permit `AllowStreamLocalForwarding`; failed host-key checks, reverse socket binding, or forwarding leave access unavailable rather than falling back to a public listener. The restricted Unix gateway never forwards the unrestricted local API and has no Gram/admin/server methods.
