...
- `endpoint` — where to reach the peer for outbound connections (consumed by later federation work).
- `expected_node_id` — the node identity to verify on connect (consumed by later federation work).
