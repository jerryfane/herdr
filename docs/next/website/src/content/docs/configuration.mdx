...
- `token_file` — path to the shared secret. Its trimmed contents are the token this peer presents; the file's bytes are never logged. A peer with no readable, non-empty token file is skipped.
- `endpoint` — where to reach the peer for outbound connections.
- `expected_node_id` — an optional install-identity pin checked both when the peer connects inbound and when its outbound `agent.list` response is polled. Missing or changed identities are rejected while the pin is set.
- `capability` — the tier this peer's token grants over inbound TCP federation. One of `observe`, `interact`, or `admin`. Default: `observe`.
