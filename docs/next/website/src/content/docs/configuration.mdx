...
- `alias` — a local name for the peer, used in logs and diagnostics.
...
The table key is the saved machine's immutable profile ID. The endpoint catalog remains authoritative for its label, SSH target, remote session, and enabled state; renaming therefore cannot change routing identity. Removing and re-adding a machine creates a new profile ID and does not inherit the old trust policy. `expected_machine_id` is required, and unknown credential fields are rejected. OpenSSH continues to own authentication and SSH federation remains a full-control trust boundary; this policy never stores or derives tokens, passwords, or private keys.
