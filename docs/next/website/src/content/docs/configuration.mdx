...
- `token_file` — path to the shared secret used by inbound and outbound TCP federation. Its trimmed contents are never logged. An unreadable or empty file grants no inbound access and sends no outbound TCP credential. SSH peers authenticate through OpenSSH instead.
- `endpoint` — where to reach the peer for outbound connections: `tcp://host:port` or `ssh://[user@]host`.
- `profile_id` — the immutable 32-character saved-machine profile ID used to key SSH executable metadata and bridge lifetime. Required for SSH endpoints.
- `remote_session` — the explicit named Herdr session served by `remote-api-bridge`. Required for SSH endpoints.
...
- `capability` — the tier a token grants over TCP federation. One of `observe`, `interact`, or `admin`. Default: `observe`. SSH remains a full-control trust boundary.

Saved-machine federation trust is separate from both the endpoint catalog and explicit peer endpoints:
