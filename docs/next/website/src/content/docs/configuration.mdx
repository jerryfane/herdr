...
The reverse socket is private to the remote account and reconnects with the saved SSH session. While disconnected, remote Gram operations fail instead of silently writing to a second local store. Files are staged with the existing size limits, then committed to the coordinator only after their length and SHA-256 match; downloads verify both before atomically replacing the destination. A deleted source attachment cannot be downloaded, and an interrupted upload leaves only bounded staging that is swept after its expiry.
