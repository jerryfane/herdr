...
Use `herdr machine status` (or `herdr machine status --json`) on the coordinator to inspect each saved profile without conflating the independent planes. The result reports saved-profile state, endpoint/TUI status when the coordinator has one, federation reachability, the validated remote machine ID, last successful poll time, a bounded error class, negotiated API version/capabilities, and whether cached data is stale. A saved machine can therefore remain enabled while its federation API is degraded or unreachable. Error details never include SSH command lines, token material, or remote stderr.

### Migrating SSH federation peers

For an SSH peer that duplicates a saved machine, remove its `[[federation.peers]]` table, enable `federation.coordinator`, and add one `federation.saved_machines."<profile-id>"` policy keyed by the existing immutable profile ID. Keep `expected_machine_id` pinned to the remote daemon's install identity. Do not use the mutable label as an alias or recreate the profile: removing and re-adding it produces a new ID and intentionally drops the old trust policy.
