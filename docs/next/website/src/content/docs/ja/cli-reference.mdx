...
SSH 鍵を手作業でコピーする代わりに、QR コードを読み取って herdr アプリをこのマシンに接続します。

```bash
herdr pair                 # このマシンの Tailscale アドレスに紐づく QR を表示
herdr pair --lan           # 代わりにプライベート LAN でペアリング（Linux のみ、RFC1918 アドレスのみ）
herdr pair --ttl 120       # コードの有効時間（既定 300 秒、最大 3600 秒）
herdr pair --port 4021     # OS 任せではなく固定ポートで待ち受ける
```

スマートフォンは自分で SSH 鍵ペアを生成し、**公開鍵**だけを送信します。公開鍵は `~/.ssh/authorized_keys` に追記されます。このコマンドが秘密鍵を表示・転送・保存することは一切ありません。

**ペアリングの前に、マシンの SSH サーバーが動作している必要があります。** ペアリングは `~/.ssh/authorized_keys` に鍵を追記するだけで、サーバーの起動や設定は行いません。SSH が無効な場合、ペアリング自体は成功しますが、その後アプリから接続できません。macOS では既定で無効です。**システム設定 → 一般 → 共有 → リモートログイン**を有効にしてください。Linux では、ディストリビューションの手順に従って OpenSSH サーバーをインストールし、起動してください。systemd 環境では、Debian または Ubuntu なら `systemctl status ssh`、Fedora または RHEL なら `systemctl status sshd` で確認できます。

ペアリングは Tailscale アドレスにバインドするため、待ち受けがインターネットから到達可能になることはありません。Tailscale がない場合、`--lan` を渡さない限りバインドを拒否します。`--lan` はプライベートアドレスのみを受け付け、公開アドレスはいずれの場合も拒否されます。Tailscale が利用可能なときは常にそちらが優先されるため、フラグを渡しても知らないうちに弱い経路へ切り替わることはありません。

`--lan` は現在 **Linux でのみ**動作します。アドレスの解決に `hostname -I` を使用しますが、BSD および macOS の `hostname` はこのオプションに対応していないため、macOS ではアドレスが見つからずペアリングが拒否されます。また、そのコマンドが列挙する**最初**のプライベートアドレスを選ぶため、インターフェースが複数あるマシンでは意図したものとは限りません。`--port` を使い、スキャンする前に表示されたアドレスを確認してください。

コードは一度きりの使い捨てで、自動的に期限切れになります。したがって、使用済みのコードを撮影した写真には何の価値もありません。Herdr が読み取れた場合に限り、QR にはこのマシンの SSH ホスト鍵フィンガープリントも含まれます。これにより、アプリは最初の接続前にホストを固定できます。読み取れない場合、`herdr pair` はホスト鍵を `UNKNOWN` と表示し、アプリは代わりに最初に提示された鍵を信頼します。

この方法で追加された鍵にはすべて印が付くので、取り消す前に対象のエントリを見つけられます。

```bash
grep herdr-pair ~/.ssh/authorized_keys
```

`grep` は一致する行を表示するだけで、アクセスを取り消しません。デバイスのアクセスを取り消すには、`~/.ssh/authorized_keys` を開き、そのデバイスに対応する `herdr-pair` 行全体を削除して保存します。関係のない行は変更しないでください。
...
Gram は Herdr アプリに表示される、オーナーとエージェント間のメッセージチャネルです。

```bash
herdr gram send "<text>" [--from LABEL] [--file PATH]  # オーナーにメッセージを送る(プッシュ通知)
herdr gram list [--queue] [--unread] [--owner]  # --owner はオーナーとして読む
herdr gram grab <id> [--as LABEL]               # 共有キューの項目を取得する(先着順)
herdr gram get-file <id> -o PATH                # メッセージの添付ファイルをダウンロードする
herdr gram post "<text>" [--to AGENT]           # オーナー:共有キュー、または特定のエージェント宛
herdr gram mark-read <id>                       # オーナー:メッセージを既読にする
herdr gram delete <id> [--owner]                # メッセージ(と添付ファイル)を完全に削除する
```

エージェントの識別子は `HERDR_PANE_ID` から解決され、エージェント名(名前がなければペイン id)になります。この名前は再起動やデプロイのハンドオフをまたいでも保持されますが、あくまで「名前」です。名前の変更・クリア・再利用により、`gram list` で「自分のもの」として表示される項目が変わります。取得(grab)自体は識別子に関わらず常に先着順でアトミックであり、2 つのエージェントが同じ項目を取得することはありません。属性表示(`from` やオーナー投稿のメッセージ)は参考情報であり、認証されたものではありません。

`send --file` はファイルを添付し(キャプションは任意)、リクエストサイズの上限に収まるようチャンク単位でアップロードします。`get-file` はダウンロードします。`delete` はメッセージと添付データを完全に削除します。使い捨ての機密情報(一時的な API キーなど)を一度だけ送って破棄する用途に便利です。削除できるのは自分が送った・取得した・自分宛てのメッセージで、`--owner` は任意のメッセージを削除します。
...
`--wait` では、最初に settled だったエージェントのライフサイクルシーケンスが進むと `delivery: "submitted"` になります。Herdr は PTY バッチに不透明な composer attempt id を付けます。同じ attempt がカーソルで裏付けられたコンポーザーに残れば `agent_prompt_unsubmitted`、観測後に消えれば `submitted` です。bracketed paste は表示された token の文字列に依存しません。

書き込まれたものの行方を証明できなければ `agent_prompt_stalled` です。すでに working の場合、進行中の別ターンの完了は新しいプロンプトに帰属されません。`pane get` の `composer.state` は `empty`、`draft_present`、`unknown` のいずれかで、provenance と補強証拠も返します。領域欠落、再描画途中、矛盾、古い daemon の応答は `unknown` です。
