...
`agent prompt` は bracketed paste モードを尊重し、テキストと Enter を PTY に書き込みます。すでに `blocked` なら書き込み前に `agent_blocked` を返します。`--wait` 指定時は、送信を確認できた場合だけ要求された状態を待ちます。確認できなければ `result.type: "agent_prompted"`、`delivery: "written_to_pty"`、終了コード 0 の受領応答を返し、エージェントが新しいターンを始めたとは判断しません。コンポーザーを観測できない場合は早く返ることがあります。同じ attempt の草稿が残っていれば `agent_prompt_unsubmitted` エラーです。`--until` には `--wait` が必要です。単独の `agent wait` は現在の状態が一致すれば即座に返り、デフォルトの対象は `idle`、`done`、`blocked` です。

送信確認後も期限までに要求した状態を観測できなければ、`agent_status_unobserved_after_submit`（終了コード 1）を返します。配送失敗ではありません。途中の `working` 状態を観測せず指定した settled 状態へ直接進んだ場合も成功です。再送しないでください。

`delivery: "submitted"` の証拠は、最初に settled だったエージェントのライフサイクルシーケンスの進行か、同じ writer-attributed composer attempt が観測後に消えたことです。すでに working のエージェントで別のターンが終わっても証拠にはなりません。証拠がなければ `written_to_pty` は PTY への書き込みだけを確認します。`pane get` は provenance とともに `composer.state` を返し、領域欠落、再描画中、矛盾、旧 daemon の応答は `unknown` です。送信を確認できない場合、状態待ちは続けません。再送前にペインを確認してください。
...
タイムアウトは PTY 書き込み前にも起こり得ます。`written_to_pty` は書き込み済みですがエージェントの送信は未確認です。再送前にペインを確認してください。

`pane wait-output` と単独の `agent wait` は、`--timeout` を省略すると無期限に待ちます。`agent prompt --wait` は送信を確認できた場合にだけ要求された安定状態を待ち、確認できない書き込みには PTY 受領応答を返します。PTY 書き込み確認前のタイムアウトはエラーです。実際の配信失敗やサーバーエラーは JSON として stderr に出力され終了コード 1、CLI 使用法のエラーは終了コード 2 です。
