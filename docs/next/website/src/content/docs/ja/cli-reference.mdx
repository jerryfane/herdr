...
これらのコマンドは、ライフサイクル状態とは独立して、進行中のモーダル入力要求も公開します。モーダルが表示されている間は `"input_pending": true` と、`confirm`、`select`、`free_text`、`unknown` のいずれかの `input_prompt_kind` を含みます。それ以外の場合、両方のフィールドは省略されます。このシグナルによって `agent_status` が変わることはありません。
