...
`agent prompt` は現在の bracketed paste モードを尊重し、working 中でもテキストとエンコード済み Enter をアトミックに送信します。`--wait` では、最初に settled だったエージェントのライフサイクルシーケンスが進むと `delivery: "submitted"` になります。正規化されたプロンプトそのものが表示された後に一致しなくなる場合も `submitted` です。Claude の `[Pasted text #N]` のようなマニフェスト定義の bracketed-paste トークンでは、書き込み前の正常なスナップショットに既存トークンがなく、配送後にトークンが表示され、その後コンポーザーが空になる必要があります。

正確なプロンプトまたは帰属可能な paste トークンが残れば `agent_prompt_unsubmitted` になります。読み取り不能なベースライン、既存トークン、または別の空でないテキストに置き換わったトークンは配送を証明せず、ほかの証拠もなければ `agent_prompt_stalled` です。すでに working の場合、進行中の別ターンの完了は新しいプロンプトに帰属されず、`submitted` にはコンポーザーがクリアされた証拠が必要です。同じペインへの並行配送に残る表示ベースの競合は [#17](https://github.com/jerryfane/herdr/issues/17) の writer-side provenance へ延期されています。その後 Herdr は要求された settled 状態を待ちます。`--until` は一致状態を絞り込み、`--wait` なしでは拒否されます。単独の `agent wait` は現在の状態が一致すれば即座に返ります。どちらもデフォルトは `idle`、`done`、`blocked` です。
