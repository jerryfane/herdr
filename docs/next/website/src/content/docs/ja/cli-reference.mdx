...
**ペアリングの前に、マシンの SSH サーバーが動作している必要があります。** ペアリングは `~/.ssh/authorized_keys` に鍵を追記するだけで、サーバーの起動や設定は行いません。SSH が無効な場合、ペアリング自体は成功しますが、その後アプリから接続できません。macOS では既定で無効です。**システム設定 → 一般 → 共有 → リモートログイン**を有効にしてください。多くの Linux ディストリビューションでは `sshd` が既に動作しています。`systemctl status ssh` で確認できます。
