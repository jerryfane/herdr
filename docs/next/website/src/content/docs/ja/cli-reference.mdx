...
コードは一度きりの使い捨てで、自動的に期限切れになります。したがって、使用済みのコードを撮影した写真には何の価値もありません。QR にはこのマシンの SSH ホスト鍵フィンガープリントも含まれるため、アプリは最初の接続前にホストを固定でき、応答してきた相手を無条件に信頼せずに済みます。

この方法で追加された鍵にはすべて印が付くので、後から見つけて取り消せます。
