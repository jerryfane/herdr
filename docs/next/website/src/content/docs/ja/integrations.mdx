...
OMP インテグレーションは、Herdr のソケット API を通じてエージェントラベル `omp`、ライフサイクル状態、正規化済みのネイティブセッションパス、選択中の会話ツリーリーフ、正確なプロセス PID を報告します。Herdr はサーバー再起動後に `omp --resume=<session>` で OMP ペインを resume でき、リーフと PID の報告を使って OMP セッション移行が要求されたブランチとプロセスを対象にしていることを検証します。
