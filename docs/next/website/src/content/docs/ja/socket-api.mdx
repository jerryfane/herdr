...
| エージェント | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.transfer_session` |
...
`gram.*` はオーナーとエージェント間のメッセージチャネルです。`gram.grab` は共有キューの項目を取得し、ストレージ層で先着順かつアトミックなので、識別子に関わらず 2 つのエージェントが同じ項目を取得することはありません。送信者・取得者の識別子は `caller_pane_id` から解決されるエージェント名(名前がなければペイン id)で、再起動やライブハンドオフをまたいでも保持されますが、名前のセマンティクスを持ちます。名前の変更・クリア・再利用により、`gram.list` で「自分のもの」として表示される項目が変わります。`gram.list` は `caller_pane_id` を省略するとオーナービューを、指定するとそのペインのエージェントビューを返します。`gram.list` は任意の `limit`(最新 N 件、上限 500 にクランプ)と `before_id`(カーソル。その id より厳密に古いものだけを返し、id は同じフィルタ済みリストに含まれていなければ `invalid_params`)でページングできます。レスポンスには `has_more` と、ページではなくフィルタ済みリスト全体で数えた `unread_count` が加わります。条件付き取得は先頭ページ専用で、`before_id` がある場合 `if_unchanged_digest` は無視され、ダイジェストは常にフィルタ済みリスト全体に対して計算されます。属性表示は参考情報であり、認証されたものではありません。

メッセージは 1 つのファイルを持てます。1 リクエストのサイズには上限があるため(アプリの SSH 経路で約 65 KiB、ローカルソケットで 1 MiB)、ファイルは `gram.upload_chunk`(`{upload_id, offset, data_base64}`。`offset` はこのチャンクの開始バイト位置で、`offset: 0` はアップロードをやり直す)でチャンク単位にアップロードし、その `upload_id` を `gram.send`/`gram.post` の `file` に渡して添付します。`gram.get_file` はレスポンス経路にサイズ上限がないため、バイト列を base64 として 1 回の応答でインライン返却します。`gram.delete` はメッセージと添付バイトを完全に削除します。オーナー(`caller_pane_id` なし)は任意のメッセージを、エージェントは caller pane を渡した場合に自分が見られるメッセージ(`gram.list` と同じ範囲)のみを削除・取得できます。この範囲チェックはフラットなローカル信頼ドメイン内の協調的フィルタリングであり、認証された境界ではありません(ローカルプロセスはストアを直接読めます)。短期的な機密(一時的な API キー)にとっての利点は、削除がバイト列を確実に消去し、削除後は永続ストアに秘密が残らないことです。

`agent.wait` はサーバー所有でイベント駆動です。解決したペイン占有者に固定されるため、別のエージェントが待機を満たすことはありません。`agent.prompt` は `until` と `timeout_ms` を持つ省略可能な `wait` オブジェクトを受け付けます。これは PTY 書き込みと送信の観測をまとめ、送信を確認できた場合だけ要求された状態を待ちます。エージェントがすでに `blocked` なら書き込まず `agent_blocked` を返します。成功した PTY 書き込みには `delivery: "written_to_pty"`、最初に settled だったエージェントの帰属可能なライフサイクル進行か、同じ writer-attributed composer attempt の観測後の消去には `delivery: "submitted"` を返します。`wait` 指定時の `written_to_pty` は送信未確認を意味し、状態待ちは続けません。再送前にペインを確認してください。同じ attempt の草稿が残る場合は `agent_prompt_unsubmitted`、モーダルが見える場合は書き込み前に `agent_input_pending` を返します。bracketed paste は表示 token に依存しません。

送信確認後、期限までに要求した状態を観測できなければ `agent_status_unobserved_after_submit` を返します。すでに送信済みのため再送しないでください。`working` の観測なしで要求した settled 状態へ直接進んでも成功です。

`agent.transfer_session` はローカルサーバー上で Claude Code、Codex、OMP のネイティブセッションを双方向に移行します。最初のリクエストは移行先を作成して実際の JSONL を再読込・検証し、元のエージェントを動かしたまま `ready` で停止します。`message_count`、`omissions`、返された `target_account` を確認後、同じ対象情報に `transfer_id` と `confirm: true` を加えて送信します。Herdr はランタイムを置き換える直前に両方のトランスクリプトのフィンガープリントを再確認します。Claude の移行先は公式インテグレーションから正確なセッションを報告する必要があります。Codex 0.150.1 は最初のプロンプトまでその報告を遅延するため、Codex の移行先では、ネイティブ移行先 JSONL と正確な `codex resume <session-id>` プロセスを検証し、選択した PID を記録することでも完了できます。OMP の移行元と移行先には最新の公式インテグレーションが必要です。インテグレーションは正規化済みのセッションパス、選択中のツリーリーフ、正確なプロセス PID を報告し、Herdr はそのリーフの表示ブランチと報告されたフォアグラウンド OMP プロセスを検証します。一定時間の観察は対話ブロッカーの検出に使いますが、証明の条件ではありません。時間が過ぎても内容を検証できれば続行し、プロセスが準備済みに見えても内容が一致しなければロールバックします。比較では表示される役割・順序・テキストの一致を必須とし、プロバイダーのメタデータ、JSON フィールド順、非表示レコード順、JSONL の CRLF 行末の違いは許容します。後から届く公式レポートも、受理済みの移行先識別子と一致しなければなりません。ブロッカー、終了、タイムアウト、内容不一致、または同一性不一致が起きると、元のハーネス、セッション、アカウント、名前、ラベル、作業ディレクトリを復元します。アカウントのファイルシステム権限を使うため federation からは利用できません。

`pane.get` は `composer` オブジェクトを返します。`state` は `empty`、`draft_present`、`unknown` で、任意の `attempt_id` と provenance、領域、カーソル、style、frame stability の証拠を含みます。領域欠落、再描画途中、矛盾、またはこのフィールドを持たない古い応答は `unknown` です。style は信頼度を下げる tripwire にしか使いません。
