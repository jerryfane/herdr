...
`agent.wait` はサーバー所有でイベント駆動です。解決したペイン占有者に固定されるため、置換されたエージェントが待機を満たすことはありません。`agent.prompt` は `until` と `timeout_ms` を持つ省略可能な `wait` オブジェクトを受け付けます。これにより、プロンプト送信と待機開始を 1 つのリクエストで行い、別々の呼び出し間の競合を避けられます。成功時の省略可能な `delivery` は、text+Enter 全体の書き込み確認後に `written_to_pty`、帰属可能なライフサイクル進行、正確なコンポーザー内容の観測後のクリア、または帰属可能なマニフェスト定義の paste トークンを観測した後にコンポーザーが空になった場合に `submitted` です。paste トークンの帰属には、書き込み前に正常に読み取れたトークンのないスナップショットが必要です。読み取り不能なベースライン、既存トークン、または別の空でないテキストは配送を証明しません。同じペインへの並行配送の帰属は [#17](https://github.com/jerryfane/herdr/issues/17) の writer-side provenance へ延期されています。
