...
`agent.transfer_session` はローカルサーバー上の Claude Code と Codex のネイティブセッションを相互に移行します。最初のリクエストは移行先を作成して実際の JSONL を再読込・検証し、元のエージェントを動かしたまま `ready` で停止します。`message_count`、`omissions`、返された `target_account` を確認後、同じ対象情報に `transfer_id` と `confirm: true` を加えて送信します。Herdr はランタイムを置き換える直前に両方のトランスクリプトのフィンガープリントを再確認します。Claude の移行先は公式インテグレーションから正確なセッションを報告する必要があります。Codex 0.150.1 は最初のプロンプトまでその報告を遅延するため、Codex の移行先では、ネイティブ移行先 JSONL と正確な `codex resume <session-id>` プロセスを検証し、選択した PID を記録することでも完了できます。3 秒の観察時間は対話ブロッカーの検出に使いますが、証明の条件ではありません。時間が過ぎても内容を検証できれば続行し、プロセスが準備済みに見えても内容が一致しなければロールバックします。比較では表示される役割・順序・テキストの一致を必須とし、プロバイダーのメタデータ、JSON フィールド順、非表示レコード順、JSONL の CRLF 行末の違いは許容します。Codex から後で届く最初の公式レポートも一致しなければなりません。ブロッカー、終了、タイムアウト、内容不一致、または同一性不一致が起きると、元のハーネス、セッション、アカウント、名前、ラベル、作業ディレクトリを復元します。アカウントのファイルシステム権限を使うため federation からは利用できません。
