...
`agent.wait` はサーバー所有でイベント駆動です。解決したペイン占有者に固定されるため、置換されたエージェントが待機を満たすことはありません。`agent.prompt` は `until` と `timeout_ms` を持つ省略可能な `wait` オブジェクトを受け付けます。成功時の `delivery` は、text+Enter 全体の書き込み確認後に `written_to_pty`、帰属可能なライフサイクル進行、または同じ writer-attributed composer attempt の観測後の消去で `submitted` です。残存する帰属入力は `agent_prompt_unsubmitted`、不確かな行方は `agent_prompt_stalled` です。bracketed paste は表示 token に依存しません。
