...
| エージェント | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.acp_register`、`agent.acp_endpoint`、`agent.acp_status`、`agent.acp_attach`、`agent.acp_detach` |
...
`agent.acp_*` メソッドはローカルの非公開 coordinator 境界を構成します。`agent.acp_register` は固定 adapter、cwd、identity を検証した後、空の shell pane を新しい ACP 所有 worker として確保します。`agent.acp_endpoint` はこの所有クラスに限り、10 秒有効で一度だけ使える `herdr agent acp-attach` 起動 ticket を返します。通常の PTY agent は `acp_ownership_required` になります。`agent.acp_status` は ticket を発行しない readiness check で、ticket、lease、adapter command を公開しません。endpoint、status、attach、detach は placeholder が引き続き shell 所有であることを再検証します。attach と detach は非公開 credential を消費し、すべての操作を worker generation で fence し、foreground ownership、identity、cwd、topology、lifecycle が変われば fail closed になります。これらのメソッドによってネイティブ PTY session id が新しい adapter process に移植可能になることはありません。
