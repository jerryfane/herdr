...
`agent prompt --wait` は、エージェントがすでに `blocked` なら PTY に書き込まず `agent_blocked` を返します。それ以外はプロンプトを書き込み、送信された証拠を調べます。送信を確認できた場合だけ指定した状態を待ちます。確認できない場合は、エラーではなく `type: "agent_prompted"`、`delivery: "written_to_pty"`、終了コード 0 を返します。コンポーザーを観測できないエージェントでは、この応答をすぐ返す場合があります。これはエージェントがターンを開始した証明ではありません。同じ attempt の草稿がコンポーザーに残る場合は `agent_prompt_unsubmitted`、PTY 書き込み前の失敗はエラーです。単独の `agent wait` は現在の状態を観測し、一致していればすぐ返ります。デフォルトの対象は `idle`、`done`、`blocked` です。`--until` は繰り返し指定でき、`agent prompt` では `--wait` が必要です。

`delivery: "submitted"` には、この送信に帰属できる証拠が必要です。最初に settled だったエージェントのライフサイクルシーケンスが進むか、同じ writer-attributed composer attempt が観測後に消えた場合です。すでに working のエージェントで別ターンが完了しても証拠にはなりません。証拠がなければ `written_to_pty` は PTY への書き込みだけを保証します。二重送信を避けるため再試行前にペインを確認してください。観測エラーやログにプロンプト本文は含まれません。

送信を確認できても期限までに指定した状態を観測できなければ、`agent_status_unobserved_after_submit` を返します。これは配送失敗ではなく状態待ちのエラーです。`working` の観測なしで指定した settled 状態へ直接進んだ場合は成功とみなします。状態待ちのエラーを理由に再送しないでください。

`pane get` の `composer.state` は `empty`、`draft_present`、`unknown` のいずれかで、`attempt_id` と `evidence` が根拠を示します。Herdr が書いていない、カーソルより右側だけの候補テキストは draft ではありません。領域が見つからない、再描画途中、証拠が矛盾する、または古い daemon が `composer` を返さない場合は必ず `unknown` です。カーソルは provenance の補強にのみ使われ、色だけで確定判定はしません。

エージェントがすでに working の場合、進行中の別ターンの完了は新しいプロンプトに帰属されません。送信を確認できた場合だけ `--wait` は要求された settled 状態を待ちます。確認できない場合は `written_to_pty` を返し、状態待ちは続けません。因果関係を推定せず状態だけを観測するには別途 `agent wait` を使ってください。デフォルトの対象は `idle`、`done`、`blocked` です。複数の状態には `--until idle --until done` のように `--until` を繰り返し、`unknown` は明示してください。`agent prompt` では `--until` に `--wait` が必要です。
...
タイムアウトは PTY 書き込み前にも起こり得ます。`written_to_pty` は書き込み済みだがエージェントの送信は未確認という意味です。二重送信を避けるため、再試行の前にペインを確認してください。ID とエージェント名はサーバーごとに独立しています。TUI で別のマシンを選んでも、既存ペイン内の CLI コマンドの接続先は変わりません。
