...
`agent prompt --wait` は PTY の確認応答後に Herdr が証明できる配送状態を区別します。最初に settled だったエージェントでは、ライフサイクルシーケンスが進むと `delivery: "submitted"` になります。コンポーザーの証拠でも配送を証明できます。正規化されたプロンプトそのものが表示された後に一致しなくなるか、Claude の `[Pasted text #N]` のようなマニフェスト定義の bracketed-paste トークンが表示された後にコンポーザーが空になる必要があります。paste トークンをこの配送のものと判断できるのは、書き込み前のスナップショットを正常に読み取れ、既存トークンがなかった場合だけです。正確なプロンプトまたは帰属可能なトークンが残れば `agent_prompt_unsubmitted`、読み取り不能なベースライン、既存トークン、または別の空でないテキストに置き換わったトークンは配送の肯定的証拠になりません。それ以外で書き込み後の行方を観測できなければ `agent_prompt_stalled` になります。

エージェントがすでに working の場合、進行中の別ターンの完了は新しいプロンプトに帰属されません。`submitted` にはコンポーザーが観測され、その後クリアされた証拠が必要で、それ以外の成功は `written_to_pty` のままです。同じペインへの並行配送には、表示の出現に基づく競合が残ります。この仕組みは [#17](https://github.com/jerryfane/herdr/issues/17) の writer-side provenance に置き換えられる予定です。配送確認の後、Herdr は要求された settled 状態を待ちます。単独の `agent wait` は現在のエージェントを監視し、すでに状態が一致していれば即座に返ります。どちらもデフォルトでは `idle`、`done`、`blocked` を待ちます。複数の正確な状態を許可するには、`--until idle --until done` のように `--until` を繰り返します。`unknown` が必要な場合は `--until unknown` を明示してください。`agent prompt` では `--until` に `--wait` が必要です。
