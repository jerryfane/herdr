...
稼働中のサーバーがライブハンドオフを拒否することもあります。ハンドオフは新しいプロセスを起動してから古いプロセスを終了させる仕組みのため、その終了によって新しいプロセスまで巻き添えになる場合、Herdr はハンドオフを拒否します。特に Herdr が systemd ユニットのメインプロセスである場合が該当します。メインプロセスが終了するとユニットが停止し、そのコントロールグループにあるすべてのペインが強制終了されるからです。このときリクエストは明示的なエラーで失敗し、何も破棄されません。

サービス管理下のインストールへ配布する場合は、バイナリを置き換えてからサービスを再起動してください。なお、サービスの再起動だけではステージ済みビルドは反映されません。ハンドオフが行うはずだった入れ替えが行われないため、ユニットは同じファイルを再実行します。先にステージ済みバイナリを稼働中のパスへ移動し (実行中のイメージは使用中なので、コピーではなく rename)、その後でサービスを再起動してください。
