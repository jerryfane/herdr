...
`agent.transfer_session` supports Claude Code and Codex native sessions on the local server. The first request supplies `target`, `to`, and optional target `account`; it parses the source once, creates a native destination session, rereads the actual destination JSONL, and leaves the source running with phase `ready`. Review `message_count`, `omissions`, and the echoed `target_account` in the returned agent's `session_transfer`, then repeat the request with the same target fields plus `transfer_id` and `confirm: true`. Herdr rechecks both transcript fingerprints before replacing the runtime. Claude must report the exact staged session through its official integration. Codex 0.150.1 defers that report until the first prompt, so a Codex target may instead complete when Herdr verifies the native destination JSONL and an exact `codex resume <session-id>` process, recording the selected PID. The three-second observation window can reveal an interactive blocker but is not the proof gate: verified content proceeds even when the window expires, while a content mismatch rolls back even if the process looks ready. Comparison requires the same visible roles, order, and text, while allowing provider metadata, JSON field ordering, non-visible record ordering, and CRLF JSONL line endings. The first later Codex integration report must still match. Any blocker, exit, timeout, content mismatch, or identity mismatch restores the original harness, session, account, name, label, and working directory. The method is intentionally unavailable over federation because it has selected-account filesystem authority.
