...
A message may carry one file. Because a single request is size-capped (near 65 KiB over the app's SSH path, 1 MiB on the local socket), a file is uploaded in chunks with `gram.upload_chunk` (`{upload_id, offset, data_base64}`; `offset` is the byte position this chunk starts at, and `offset: 0` restarts the upload) and then attached by passing that `upload_id` in a `gram.send`/`gram.post` `file` object. `gram.get_file` returns the bytes inline as base64 in one reply (the response path is not size-capped). `gram.delete` removes a message and its file bytes for good: the owner (no `caller_pane_id`) may delete any message; an agent may delete only one it sent, grabbed, or that is addressed to it. Deletion genuinely purges the bytes, so gram is safe for a short-lived secret such as a temporary API key.
