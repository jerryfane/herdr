...
Stage and review a Claude Code/Codex session transfer, then confirm it:

```bash
herdr agent transfer-session jarvis --to codex --account codex-work
herdr agent transfer-session jarvis --to codex --account codex-work --confirm <transfer-id>
```

Use `--yes` for a one-command stage, verify, and confirm flow.
...
| Notification | `notification.show`, `notifications.register_device` |
| Gram | `gram.send`, `gram.post`, `gram.list`, `gram.grab`, `gram.mark_read`, `gram.delete`, `gram.upload_chunk`, `gram.get_file` |
...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.start`, `agent.transfer_session`, `agent.view.set`, `agent.view.clear` |
...
`gram.*` is the owner↔agent message channel. `gram.grab` claims a shared queue item and is first-wins atomic at the storage layer, so no two agents can ever claim the same item — regardless of identity. Sender and claimant identity is the caller's agent name (or pane id if it has no name), resolved from `caller_pane_id`; it is durable across a restart or live-handoff but carries name-semantics: renaming, clearing, or reusing a name changes which items an agent sees as its own in `gram.list`. `gram.list` returns the owner view when `caller_pane_id` is omitted and that pane's agent view otherwise. `gram.list` pages with the optional `limit` (the newest N, clamped to 500) and `before_id` (a cursor: strictly older than that id, which must itself be in the same filtered list, else `invalid_params`); the reply adds `has_more` and an `unread_count` computed over the whole filtered list rather than the page. Conditional fetch is head-only: `if_unchanged_digest` is ignored when `before_id` is present, and the digest always covers the full filtered list. `gram.list` pages with the optional `limit` (the newest N, clamped to 500) and `before_id` (a cursor: strictly older than that id, which must itself be in the same filtered list, else `invalid_params`); the reply adds `has_more` and an `unread_count` computed over the whole filtered list rather than the page. Conditional fetch is head-only: `if_unchanged_digest` is ignored when `before_id` is present, and the digest always covers the full filtered list. Attribution is advisory, not authenticated.

A message may carry one file. Because a single request is size-capped (near 65 KiB over the app's SSH path, 1 MiB on the local socket), a file is uploaded in chunks with `gram.upload_chunk` (`{upload_id, offset, data_base64}`; `offset` is the byte position this chunk starts at, and `offset: 0` restarts the upload) and then attached by passing that `upload_id` in a `gram.send`/`gram.post` `file` object. `gram.get_file` returns the bytes inline as base64 in one reply (the response path is not size-capped). `gram.delete` removes a message and its file bytes for good. The owner (no `caller_pane_id`) may delete any message; an agent that supplies its caller pane may delete or download only a message it can see (the same audience as `gram.list`). That audience check is **cooperative filtering within the flat local trust domain, not an authenticated boundary** — every local process can already read the store directly, and "owner" is just the absence of a caller pane. Its value for a short-lived secret (a temporary API key) is that deletion genuinely purges the bytes, keeping the secret out of the persistent store once you delete it.

`agent.wait` is server-owned and event-driven. It pins the resolved pane occupant so a replacement cannot satisfy the wait. `agent.prompt` accepts an optional `wait` object with `until` and `timeout_ms`; this combines a prompt write with submission observation and, only after verified submission, a status wait. If the resolved agent is already `blocked`, it returns `agent_blocked` before writing. Successful prompts include `delivery: "written_to_pty"` after a complete text+Enter PTY write, or `delivery: "submitted"` after an attributable lifecycle advance from a settled agent or an observed same-attempt composer clear. A successful `written_to_pty` with `wait` means submission was not verified; the request does not continue waiting for status, and callers must inspect the pane before resending. A persistent same-attempt draft returns `agent_prompt_unsubmitted`; a visible modal returns `agent_input_pending` before any write. Bracketed paste relies on writer provenance rather than rendered token text.

If submission was confirmed but a requested status is not observed by the deadline, the error is `agent_status_unobserved_after_submit`; the prompt was submitted, so clients must not resend it. A direct transition to a requested settled status is accepted without requiring a sampled `working` frame.

`agent.transfer_session` supports bidirectional native-session transfer among Claude Code, Codex, and OMP on the local server. The first request supplies `target`, `to`, and optional target `account`; it parses the source once, creates a native destination session, rereads the actual destination JSONL, and leaves the source running with phase `ready`. Review `message_count`, `omissions`, and the echoed `target_account` in the returned agent's `session_transfer`, then repeat the request with the same target fields plus `transfer_id` and `confirm: true`. Herdr rechecks both transcript fingerprints before replacing the runtime. Claude must report the exact staged session through its official integration. Codex 0.150.1 defers that report until the first prompt, so a Codex target may instead complete when Herdr verifies the native destination JSONL and an exact `codex resume <session-id>` process, recording the selected PID. OMP sources and targets require the current official integration, which reports the canonical session path, selected tree leaf, and exact process PID; Herdr verifies that leaf's visible branch and the reported foreground OMP process. The bounded observation window can reveal an interactive blocker but is not the proof gate: verified content proceeds when the window expires, while a content mismatch rolls back even if the process looks ready. Comparison requires the same visible roles, order, and text, while allowing provider metadata, JSON field ordering, non-visible record ordering, and CRLF JSONL line endings. A later official report must still match the accepted target identity. Any blocker, exit, timeout, content mismatch, or identity mismatch restores the original harness, session, account, name, label, and working directory. The method is intentionally unavailable over federation because it has selected-account filesystem authority.

`pane.get` returns an additive `composer` object with `state` (`empty`, `draft_present`, or `unknown`), optional `attempt_id`, and provenance, region, cursor, style, and frame-stability evidence. Suggestion text drawn by a hosted CLI is not a draft because it lacks writer provenance and is strictly right of the cursor. Region-not-found, mid-repaint, conflicting evidence, and old responses without the object resolve to `unknown`, not `empty`. Style is only a tripwire that can downgrade confidence.
...
Those records also expose modal input independently of lifecycle state. `input_pending: true` is paired with `input_prompt_kind` (`confirm`, `select`, `free_text`, or `unknown`); both fields are omitted when no modal is present. `pane.agent_status_changed` carries the same tuple and emits on input-only changes even when `agent_status` is unchanged. Unfiltered subscriptions provide this orthogonal transition stream, and `pane.get` / `agent.get` recover current state after reconnecting.
