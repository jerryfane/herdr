...
`agent.wait` is server-owned and event-driven. It pins the resolved pane occupant so a replacement cannot satisfy the wait. `agent.prompt` accepts an optional `wait` object with `until` and `timeout_ms`; this submits the prompt and starts the wait in one request, avoiding a race between separate calls. Successful prompts include optional `delivery` evidence: `written_to_pty` after the complete text+Enter batch is acknowledged, or `submitted` after an attributable lifecycle advance, an exact composer-observed-then-cleared transition, or an attributable manifest-declared paste token followed by an empty composer. Paste-token attribution requires a readable token-free pre-write snapshot; an unreadable baseline, a pre-existing token, or non-empty replacement text does not prove delivery. Same-pane concurrent delivery attribution remains deferred to writer-side provenance in [#17](https://github.com/jerryfane/herdr/issues/17). A visible modal returns `agent_input_pending` before any bytes are written.
