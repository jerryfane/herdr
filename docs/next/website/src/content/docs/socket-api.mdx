...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.start`, `agent.acp_register`, `agent.acp_endpoint`, `agent.acp_status`, `agent.acp_attach`, `agent.acp_detach`, `agent.view.set`, `agent.view.clear` |
...
The `agent.acp_*` methods form a local, private coordinator boundary. `agent.acp_register` claims an empty shell pane for a new ACP-owned worker after validating its fixed adapter, cwd, and identity. `agent.acp_endpoint` returns a ten-second, one-shot `herdr agent acp-attach` launch ticket only for that ownership class; ordinary PTY agents return `acp_ownership_required`. `agent.acp_status` is a non-ticketing readiness check and exposes no ticket, lease, or adapter command. Endpoint, status, attach, and detach revalidate that the placeholder remains shell-owned. Attach and detach consume private credentials, fence every operation with the worker generation, and fail closed after foreground ownership, identity, cwd, topology, or lifecycle changes. These methods do not make native PTY session ids portable to a new adapter process.
