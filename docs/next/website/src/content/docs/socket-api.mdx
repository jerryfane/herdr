...
`agent.wait` is server-owned and event-driven. It pins the resolved pane occupant so a replacement cannot satisfy the wait. `agent.prompt` accepts an optional `wait` object with `until` and `timeout_ms`; this submits the prompt and starts the wait in one request, avoiding a race between separate calls. Successful prompts include optional `delivery` evidence: `written_to_pty` after the complete text+Enter batch is acknowledged, or `submitted` after an attributable lifecycle advance or after the same writer-attributed composer attempt is observed and clears. Persistent attributed input returns `agent_prompt_unsubmitted`; uncertain disposition returns `agent_prompt_stalled`. Bracketed paste uses writer provenance and does not depend on rendered token text. A visible modal returns `agent_input_pending` before any bytes are written.
