...
The code is single-use and expires on its own, so a photograph of it is worthless once redeemed. The QR also carries this machine's SSH host-key fingerprint, which lets the app pin the host before its first connection rather than trusting whatever answers.

Every key added this way is tagged so you can find and revoke it:
