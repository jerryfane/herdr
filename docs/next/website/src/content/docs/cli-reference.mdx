...
With `--wait`, a settled agent's lifecycle-sequence advance reports `delivery: "submitted"`. Composer evidence also reports `submitted` after either the exact normalized prompt appears and stops matching, or a manifest-declared bracketed-paste token such as Claude's `[Pasted text #N]` appears and then the composer becomes empty. Paste-token attribution requires a successful pre-write snapshot with no token already present. A persistent exact prompt or attributable token returns `agent_prompt_unsubmitted`; an unreadable baseline, a pre-existing token, or a token replaced by different non-empty composer text does not prove delivery. A written but otherwise unobservable disposition returns `agent_prompt_stalled`.

For an already-working agent, an unrelated lifecycle completion is not attributed to the new prompt: composer-clear evidence is required for `submitted`, otherwise delivery remains `written_to_pty`. Same-pane concurrent deliveries remain an appearance-based race deferred to writer-side provenance in [#17](https://github.com/jerryfane/herdr/issues/17). After this delivery check, Herdr waits for the first requested settled status. `--until` narrows the matching states and is rejected unless `--wait` is also present. Standalone `agent wait` returns immediately when the current status matches. Both default to `idle`, `done`, or `blocked`; use `--until unknown` explicitly when needed.
