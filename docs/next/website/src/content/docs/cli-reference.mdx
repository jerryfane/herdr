...
Connect the herdr app to this machine by scanning a QR code, instead of copying an SSH key by hand.

```bash
herdr pair                 # print a QR bound to this machine's Tailscale address
herdr pair --lan           # pair over a private LAN instead (Linux only; RFC1918 addresses only)
herdr pair --ttl 120       # seconds the code stays valid (default 300, max 3600)
herdr pair --port 4021     # listen on a fixed port instead of one the OS picks
```

The phone generates its own SSH keypair and sends only the **public** half, which is appended to `~/.ssh/authorized_keys`. No private key is ever displayed, transported, or stored by this command.

**Your machine's SSH server must be running before you pair.** Pairing only adds a key to `~/.ssh/authorized_keys`; it does not start or configure a server. If SSH is off, pairing succeeds and the app then cannot connect. On macOS this is off by default — enable **System Settings → General → Sharing → Remote Login**. On Linux, install and start the OpenSSH server using your distribution's instructions. On systemd systems, check `systemctl status ssh` on Debian or Ubuntu and `systemctl status sshd` on Fedora or RHEL.

Pairing binds to your Tailscale address, so the listener is not reachable from the internet. Without Tailscale it refuses to bind unless you pass `--lan`, and `--lan` accepts only private addresses — a public one is refused either way. Tailscale is preferred whenever it is available, so passing the flag cannot silently move you onto a weaker path.

`--lan` currently works on **Linux only** — it resolves the address with `hostname -I`, which BSD and macOS `hostname` do not support, so on macOS it finds no address and pairing is refused. It also picks the **first** private address that command lists, which is not necessarily the interface you meant on a machine with several; use `--port` and check the printed address before scanning.

The code is single-use and expires on its own, so a photograph of it is worthless once redeemed. The QR also carries this machine's SSH host-key fingerprint when Herdr can read one, which lets the app pin the host before its first connection. If it cannot, `herdr pair` reports the host key as `UNKNOWN` and the app trusts the first key it is offered instead.

Every key added this way is tagged so you can locate the entry before revoking it:

```bash
grep herdr-pair ~/.ssh/authorized_keys
```

`grep` only displays matching lines; it does not revoke access. To revoke a device, open `~/.ssh/authorized_keys`, delete the complete `herdr-pair` line for that device, and save the file. Leave every unrelated line unchanged.
...
Gram is the owner↔agent message channel surfaced in the Herdr app.

```bash
herdr gram send "<text>" [--from LABEL] [--file PATH]  # message the owner (push-notified)
herdr gram list [--queue] [--unread] [--owner]  # --owner reads as the owner
herdr gram grab <id> [--as LABEL]               # claim a shared queue item (first-wins)
herdr gram get-file <id> -o PATH                # download a message's attached file
herdr gram post "<text>" [--to AGENT]           # owner: shared queue, or one agent
herdr gram mark-read <id>                        # owner: mark a message read
herdr gram delete <id> [--owner]                # delete a message (and any file) for good
```

An agent's identity comes from `HERDR_PANE_ID` and resolves to its agent name (or the pane id if it has no name). The name is durable across a restart or deploy handoff, but it is a name: renaming, clearing, or reusing it changes which items show as "mine" in `gram list`. The grab itself is always first-wins atomic regardless of identity — two agents can never claim the same item. Attribution (`from`, and owner-posted messages) is advisory, not authenticated.

`send --file` attaches a file (with optional caption text), uploaded in chunks so it fits the request-size limits; `get-file` downloads one. `delete` removes a message and any attached bytes for good — useful for a short-lived secret you send once and then discard. You can delete a message you sent, grabbed, or that is addressed to you; `--owner` deletes any message.
...
`pane get`, `pane list`, `agent get`, and `agent list` also expose a live modal input request independently of lifecycle status. While a modal is present they include `"input_pending": true` and `input_prompt_kind` as `confirm`, `select`, `free_text`, or `unknown`; both fields are omitted otherwise. `agent_status` is not changed by this signal.
...
`agent prompt` honors live bracketed-paste mode and sends text followed by encoded Enter after a short delay, including while the agent is working. Herdr acknowledges only after the complete batch reaches the pane PTY; non-wait success reports `delivery: "written_to_pty"`. If the agent is already `blocked`, it returns `agent_blocked` without sending input. If a modal `input_pending` request is already visible, it returns `agent_input_pending` and writes zero bytes.

With `--wait`, an accepted prompt sent from another non-working state must produce an observed lifecycle change within five seconds or Herdr returns `agent_prompt_stalled`; a caller timeout of five seconds or less keeps the normal `timeout` error. After activity is observed, it waits for the first requested settled status. It does not track individual turns. If the agent is already working, completion of that active turn may satisfy the wait. `--until` narrows the matching states and is rejected unless `--wait` is also present. Standalone `agent wait` returns immediately when the current status matches. Both default to `idle`, `done`, or `blocked`; use `--until unknown` explicitly when needed.

With `--wait`, a settled agent's lifecycle-sequence advance reports `delivery: "submitted"`. Herdr also tags the PTY batch with an opaque composer attempt id. The same writer-attributed attempt remaining in a cursor-corroborated composer returns `agent_prompt_unsubmitted`; after it is observed and clears, delivery is `submitted`. Bracketed paste uses this provenance and does not depend on rendered paste-token text. A written but otherwise unobservable disposition returns `agent_prompt_stalled`.

For an already-working agent, an unrelated lifecycle completion is not attributed to the new prompt: the same observed composer attempt must clear for `submitted`, otherwise delivery remains `written_to_pty`. `pane get` reports `composer.state` as `empty`, `draft_present`, or `unknown`, plus provenance and corroborating evidence. Missing regions, repainting frames, conflicts, and older-daemon responses are `unknown`; suggestion text that Herdr never wrote is not a draft.
