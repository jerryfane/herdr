...
The `submit-verify` column (and `submission_verification_supported` in `--json`) reports whether each active manifest declares `[composer]`. It does not guarantee that a particular pane is observable at runtime.
...
herdr gram list [--queue] [--unread] [--owner] [--limit N]  # --owner reads as the owner; --limit: newest N only
...
herdr pane report-agent-session <pane_id> \
  ...
  [--agent-session-id ID] \
  [--agent-session-path PATH] \
  [--agent-session-cursor ID] \
  [--agent-process-pid PID] \
...
`report-agent-session` updates native session identity without reporting lifecycle state. Integrations for tree-shaped session logs can also report the selected leaf with `--agent-session-cursor`; `--agent-process-pid` binds that report to the exact harness process. `release-agent` ends that source's lifecycle authority when its agent process exits.
...
With `--wait`, Herdr checks the result of the acknowledged PTY write before waiting for the requested status. If submission is verified, it waits for the first matching settled status. If submission cannot be verified, it returns `result.type: "agent_prompted"` with `delivery: "written_to_pty"` and exit code 0 without claiming the agent began a turn. An agent whose manifest has no composer coverage can return that receipt promptly. A visible same-attempt draft returns `agent_prompt_unsubmitted` as an error; failures before the write remain errors. `--until` requires `--wait`. Standalone `agent wait` returns immediately when the current status matches. Both commands default to `idle`, `done`, or `blocked`; request `unknown` explicitly.

After confirmed submission, a requested status may still miss the deadline. That returns `agent_status_unobserved_after_submit` (exit 1); it does not mean the prompt was undelivered. A fast transition straight to a requested settled state is accepted without a sampled `working` state.

`delivery: "submitted"` requires evidence attributable to this send: a lifecycle-sequence advance from an initially settled agent, or an observed writer-attributed composer attempt that subsequently clears. An unrelated completion on an already-working agent does not count. Bracketed paste uses writer provenance rather than rendered paste-token text. Without submission evidence, `written_to_pty` confirms the bytes reached the pane, not that the agent accepted a new turn. Inspect the pane before resending to avoid duplicates.

`pane get` reports `composer.state` as `empty`, `draft_present`, or `unknown`, plus provenance and corroborating evidence. Missing regions, repainting frames, conflicts, and older-daemon responses are `unknown`; suggestion text that Herdr never wrote is not a draft. After verified submission, `--wait` waits for the requested settled status. When submission is unverified, use a separate `agent wait` to observe status without attributing that transition to the prompt.
...
A timeout can occur before a PTY write; `written_to_pty` confirms the write but not agent submission. Inspect the pane before retrying.

`pane wait-output` and standalone `agent wait` wait indefinitely when `--timeout` is omitted. For `agent prompt --wait`, a verified submission can continue waiting for a requested settled status without a timeout; an unverified write returns the PTY receipt instead. A timeout before PTY write confirmation remains an error. Genuine delivery and server errors appear as JSON on stderr with exit status 1; CLI usage errors exit with status 2.
