...
With `--wait`, a settled agent's lifecycle-sequence advance or an exact composer fingerprint observed and then cleared reports `delivery: "submitted"`. Persistent exact composer text returns `agent_prompt_unsubmitted`; a written but otherwise unobservable disposition returns `agent_prompt_stalled`. For an already-working agent, an unrelated lifecycle completion is not attributed to the new prompt: composer-clear evidence is required for `submitted`, otherwise delivery remains `written_to_pty`. After this delivery check, Herdr waits for the first requested settled status. `--until` narrows the matching states and is rejected unless `--wait` is also present. Standalone `agent wait` returns immediately when the current status matches. Both default to `idle`, `done`, or `blocked`; use `--until unknown` explicitly when needed.
