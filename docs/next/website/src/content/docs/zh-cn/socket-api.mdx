...
一条消息可以携带一个文件。由于单个请求有大小上限(应用的 SSH 路径约 65 KiB,本地套接字为 1 MiB),文件通过 `gram.upload_chunk`(`{upload_id, offset, data_base64}`;`offset` 为该分块的起始字节位置,`offset: 0` 表示重新开始上传)分块上传,再把该 `upload_id` 传入 `gram.send`/`gram.post` 的 `file` 完成附加。`gram.get_file` 在一次响应中以 base64 内联返回全部字节(响应路径没有大小上限)。`gram.delete` 会彻底删除消息及其附件字节:所有者(无 `caller_pane_id`)可删除任意消息,智能体只能删除自己发送的、认领的或发给自己的消息。删除会真正清除字节,因此 gram 适合承载临时 API 密钥这类短期机密。
