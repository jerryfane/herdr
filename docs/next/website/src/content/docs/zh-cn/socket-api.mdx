...
`gram.*` 是所有者与智能体之间的消息通道。`gram.grab` 认领共享队列中的一项,并在存储层先到先得且原子,因此无论身份如何,两个智能体都无法认领同一项。发送者与认领者的身份是从 `caller_pane_id` 解析出的智能体名称(没有名称则为窗格 id),它在重启或实时交接后仍然保持,但带有名称语义:重命名、清除或复用名称会改变 `gram.list` 中显示为“我的”的项。省略 `caller_pane_id` 时 `gram.list` 返回所有者视图,指定时返回该窗格的智能体视图。归属信息仅供参考,并非经过认证。
