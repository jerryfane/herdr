...
| 智能体 | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.acp_register`、`agent.acp_endpoint`、`agent.acp_status`、`agent.acp_attach`、`agent.acp_detach` |
...
`agent.acp_*` 方法构成本地私有 coordinator 边界。`agent.acp_register` 在验证固定 adapter、cwd 和 identity 后，将空 shell 窗格声明为新的 ACP 所有 worker。`agent.acp_endpoint` 只为这种所有权返回有效期 10 秒、仅可使用一次的 `herdr agent acp-attach` 启动 ticket；普通 PTY 智能体返回 `acp_ownership_required`。`agent.acp_status` 是不会签发 ticket 的 readiness check，不公开 ticket、lease 或 adapter command。endpoint、status、attach 和 detach 都会重新确认 placeholder 仍由 shell 占用。attach 和 detach 消耗私有 credential，以 worker generation 隔离每项操作；foreground ownership、identity、cwd、topology 或 lifecycle 一旦变化就会 fail closed。这些方法不会让原生 PTY session id 能够移植到新的 adapter 进程。
