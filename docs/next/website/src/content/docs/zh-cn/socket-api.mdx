...
`agent.wait` 由服务器拥有并由事件驱动。它会固定到已解析的窗格占用者,因此替换后的智能体不能满足该等待。`agent.prompt` 接受包含 `until` 和 `timeout_ms` 的可选 `wait` 对象。成功响应中的 `delivery` 在完整 text+Enter 批次确认写入后为 `written_to_pty`;在可归因的生命周期前进，或同一个 writer-attributed composer attempt 被观察到后清空时为 `submitted`。持续存在的归属输入返回 `agent_prompt_unsubmitted`，无法确定去向时返回 `agent_prompt_stalled`。bracketed paste 不依赖渲染后的 token 文本。
