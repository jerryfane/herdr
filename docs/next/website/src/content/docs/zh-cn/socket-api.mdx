...
| 智能体 | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.transfer_session` |
...
`gram.*` 是所有者与智能体之间的消息通道。`gram.grab` 认领共享队列中的一项,并在存储层先到先得且原子,因此无论身份如何,两个智能体都无法认领同一项。发送者与认领者的身份是从 `caller_pane_id` 解析出的智能体名称(没有名称则为窗格 id),它在重启或实时交接后仍然保持,但带有名称语义:重命名、清除或复用名称会改变 `gram.list` 中显示为“我的”的项。省略 `caller_pane_id` 时 `gram.list` 返回所有者视图,指定时返回该窗格的智能体视图。`gram.list` 支持分页:可选的 `limit`(最新 N 条,上限钳制为 500)与 `before_id`(游标,只返回严格早于该 id 的消息;该 id 必须存在于同一过滤后的列表中,否则返回 `invalid_params`)。响应新增 `has_more`,以及在整个过滤后列表(而非当前页)上统计的 `unread_count`。条件请求只作用于首页:存在 `before_id` 时忽略 `if_unchanged_digest`,而摘要始终基于完整的过滤后列表计算。归属信息仅供参考,并非经过认证。

一条消息可以携带一个文件。由于单个请求有大小上限(应用的 SSH 路径约 65 KiB,本地套接字为 1 MiB),文件通过 `gram.upload_chunk`(`{upload_id, offset, data_base64}`;`offset` 为该分块的起始字节位置,`offset: 0` 表示重新开始上传)分块上传,再把该 `upload_id` 传入 `gram.send`/`gram.post` 的 `file` 完成附加。`gram.get_file` 在一次响应中以 base64 内联返回全部字节(响应路径没有大小上限)。`gram.delete` 会彻底删除消息及其附件字节。所有者(无 `caller_pane_id`)可删除任意消息;智能体在提供 caller pane 时,只能删除或下载自己可见的消息(与 `gram.list` 相同的范围)。该范围检查是扁平本地信任域内的协作式过滤,并非经过认证的边界(本地进程本就能直接读取存储)。对短期机密(临时 API 密钥)而言,其价值在于删除会真正清除字节,删除后秘密不再留存于持久化存储。

`agent.wait` 由服务器拥有并由事件驱动，且固定到已解析的窗格占用者，因此替换后的智能体不能满足等待。`agent.prompt` 接受包含 `until` 和 `timeout_ms` 的可选 `wait` 对象：它写入 PTY 并观察是否提交，只有确认提交后才等待请求的状态。若智能体已经是 `blocked`，则写入前返回 `agent_blocked`。成功写入完整 text+Enter 批次后返回 `delivery: "written_to_pty"`；最初处于稳定状态的智能体发生可归因的生命周期变化，或观察到同一个 writer-attributed composer attempt 清空后返回 `delivery: "submitted"`。带 `wait` 的 `written_to_pty` 表示未确认提交，不再继续等待状态；重试前请检查窗格。同一个 attempt 的草稿仍存在时返回 `agent_prompt_unsubmitted`；看到模态提示时，在写入前返回 `agent_input_pending`。bracketed paste 不依赖渲染的 token 文本。

确认提交后，如截止时未观察到请求的状态，则返回 `agent_status_unobserved_after_submit`；提示已提交，不应重发。直接进入请求的稳定状态也算成功，不要求先观察到 `working`。

`agent.transfer_session` 可在本地服务器上双向迁移 Claude Code、Codex 与 OMP 的原生会话。第一次请求创建目标会话、重新读取并验证实际目标 JSONL，同时保持源智能体运行，并停在 `ready`。检查 `message_count`、`omissions` 和返回的 `target_account` 后，使用相同目标参数并加入 `transfer_id` 与 `confirm: true` 再次请求。Herdr 会在替换运行时之前重新检查两份会话记录的指纹。Claude 目标必须通过官方集成报告准确的暂存会话。Codex 0.150.1 会把该报告延迟到第一个提示词，因此 Codex 目标也可以在 Herdr 验证原生目标 JSONL 和准确的 `codex resume <session-id>` 进程并记录所选 PID 后完成迁移。OMP 源和目标需要当前官方集成；该集成会上报规范化会话路径、当前选中的树叶节点和准确的进程 PID，Herdr 会验证该叶节点的可见分支以及报告的前台 OMP 进程。有界观察窗口可以暴露交互阻塞，但它不是证明门槛：窗口结束后内容已验证则继续，即使进程看似就绪，只要内容不一致也会回滚。比较要求可见消息的角色、顺序和文本完全相同，但允许提供方元数据、JSON 字段顺序、不可见记录顺序和 JSONL 的 CRLF 行尾不同。后续官方报告仍必须匹配已接受的目标身份。遇到阻塞、退出、超时、内容不一致或身份不一致时，会恢复原来的工具、会话、账号、名称、标签和工作目录。由于该方法拥有所选账号目录的文件系统权限，federation 不提供此方法。

`pane.get` 返回附加的 `composer` 对象。`state` 为 `empty`、`draft_present` 或 `unknown`，并包含可选 `attempt_id` 以及 provenance、区域、光标、style 和 frame stability 证据。区域缺失、重绘中的画面、证据冲突，或旧响应没有该对象时都解析为 `unknown`。style 只能作为降低置信度的 tripwire。
