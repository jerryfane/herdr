...
`agent.wait` 由服务器拥有并由事件驱动。它会固定到已解析的窗格占用者,因此替换后的智能体不能满足该等待。`agent.prompt` 接受可选的 `wait` 对象,其中包含 `until` 和 `timeout_ms`;这样可在一个请求中提交提示并开始等待,避免两个独立调用之间的竞态。如果解析出的智能体已经是 `blocked`,`agent.prompt` 不会发送输入或开始等待,而是返回 `agent_blocked`。成功响应中的 `delivery` 在完整 text+Enter 批次确认写入后为 `written_to_pty`;在可归因的生命周期前进，或同一个 writer-attributed composer attempt 被观察到后清空时为 `submitted`。持续存在的归属输入返回 `agent_prompt_unsubmitted`，无法确定去向时返回 `agent_prompt_stalled`。bracketed paste 不依赖渲染后的 token 文本。对于完全不暴露 composer 观察的窗格，返回 `agent_prompt_unverifiable` 而非 `agent_prompt_stalled`，以便调用方区分“已观察但无证据”与“无法观察”；两者都不能证明未送达。
