...
**配对前，你的机器必须已在运行 SSH 服务。** 配对只会把密钥追加到 `~/.ssh/authorized_keys`，并不会启动或配置任何服务。如果 SSH 未开启，配对本身会成功，但应用随后无法连接。macOS 默认关闭此功能，请启用**系统设置 → 通用 → 共享 → 远程登录**。多数 Linux 发行版已在运行 `sshd`，可用 `systemctl status ssh` 检查。
