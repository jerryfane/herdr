...
`agent prompt` 遵循 bracketed paste 模式，将文本和 Enter 写入 PTY。如果智能体已经是 `blocked`，则写入前返回 `agent_blocked`。使用 `--wait` 时，只有确认提交后才等待请求的状态；无法确认时返回 `result.type: "agent_prompted"`、`delivery: "written_to_pty"` 和退出码 0，不声称智能体已开始新轮次。没有 composer 观测能力时可能很快返回。同一个 attempt 的草稿仍留在 composer 中则是 `agent_prompt_unsubmitted` 错误。`--until` 必须与 `--wait` 同用。独立的 `agent wait` 在当前状态匹配时立即返回；默认目标是 `idle`、`done` 或 `blocked`。

确认提交后，如截止时未观察到请求的状态，则返回 `agent_status_unobserved_after_submit`（退出码 1），这不代表投递失败。直接进入请求的稳定状态也算成功，无需观察到中间的 `working` 状态。不要重发提示。

`delivery: "submitted"` 需要可归因的证据：最初处于稳定状态的智能体生命周期序列前进，或同一个 writer-attributed composer attempt 被观察到后清空。已经处于 working 的智能体完成其他轮次不算证据。没有证据时，`written_to_pty` 只确认 PTY 写入。`pane get` 返回带 provenance 的 `composer.state`；区域缺失、重绘中、证据冲突或旧 daemon 的响应均为 `unknown`。无法确认提交时，不会继续等待请求的状态；重试前先检查窗格。
...
超时可能发生在 PTY 写入前；`written_to_pty` 表示已写入，但尚未确认智能体提交。重试前请检查窗格。

`pane wait-output` 和独立的 `agent wait` 在省略 `--timeout` 时可无限等待。`agent prompt --wait` 仅在确认提交后继续等待指定的稳定状态；未确认的写入则返回 PTY 回执。PTY 写入得到确认前的超时仍是错误。真正的投递失败和服务器错误以 JSON 写到 stderr，退出码为 1；CLI 用法错误的退出码为 2。
