...
扫描二维码即可将 herdr 应用连接到本机，无需手动复制 SSH 密钥。

```bash
herdr pair                 # 打印绑定到本机 Tailscale 地址的二维码
herdr pair --lan           # 改为通过私有局域网配对（仅限 Linux，仅限 RFC1918 地址）
herdr pair --ttl 120       # 二维码的有效时长（默认 300 秒，最长 3600 秒）
herdr pair --port 4021     # 监听固定端口，而不是由系统随机选择
```

手机会自行生成 SSH 密钥对，并且只发送**公钥**，该公钥会被追加到 `~/.ssh/authorized_keys`。此命令绝不会显示、传输或存储任何私钥。

**配对前，你的机器必须已在运行 SSH 服务。** 配对只会把密钥追加到 `~/.ssh/authorized_keys`，并不会启动或配置任何服务。如果 SSH 未开启，配对本身会成功，但应用随后无法连接。macOS 默认关闭此功能，请启用**系统设置 → 通用 → 共享 → 远程登录**。在 Linux 上，请按照发行版的说明安装并启动 OpenSSH 服务器。在使用 systemd 的系统上，Debian 或 Ubuntu 可用 `systemctl status ssh` 检查，Fedora 或 RHEL 则使用 `systemctl status sshd`。

配对绑定到你的 Tailscale 地址，因此监听端口不会暴露在公网上。若没有 Tailscale，除非传入 `--lan`，否则将拒绝绑定；而 `--lan` 只接受私有地址，公网地址在任何情况下都会被拒绝。只要 Tailscale 可用就会优先使用，因此传入该标志不会在你不知情的情况下降级到更弱的路径。

`--lan` 目前**仅在 Linux 上可用**：它通过 `hostname -I` 解析地址，而 BSD 与 macOS 的 `hostname` 并不支持该选项，因此在 macOS 上找不到地址，配对会被拒绝。它还会选择该命令列出的**第一个**私有地址，在多网卡机器上未必是你想要的那一个；请配合 `--port`，并在扫描前核对打印出来的地址。

二维码只能使用一次并会自动过期，所以一旦被兑换，它的照片就毫无价值。只有在 Herdr 能够读取时，二维码才会包含本机的 SSH 主机密钥指纹，使应用能够在首次连接前固定主机。如果无法读取，`herdr pair` 会将主机密钥显示为 `UNKNOWN`，应用则会信任首次收到的密钥。

通过这种方式添加的每个密钥都会带有标记，方便你在撤销前找到对应条目：

```bash
grep herdr-pair ~/.ssh/authorized_keys
```

`grep` 只会显示匹配的行，并不会撤销访问权限。要撤销某个设备，请打开 `~/.ssh/authorized_keys`，删除该设备对应的整行 `herdr-pair` 条目并保存文件。不要修改任何无关的行。
...
Gram 是在 Herdr 应用中呈现的所有者↔智能体消息通道。

```bash
herdr gram send "<text>" [--from LABEL] [--file PATH]  # 给所有者发消息(推送通知)
herdr gram list [--queue] [--unread] [--owner]  # --owner 以所有者身份查看
herdr gram grab <id> [--as LABEL]               # 认领共享队列中的一项(先到先得)
herdr gram get-file <id> -o PATH                # 下载消息附带的文件
herdr gram post "<text>" [--to AGENT]           # 所有者:共享队列,或指定某个智能体
herdr gram mark-read <id>                       # 所有者:将消息标记为已读
herdr gram delete <id> [--owner]                # 彻底删除一条消息(及其附件)
```

智能体的身份来自 `HERDR_PANE_ID`,解析为其智能体名称(没有名称则为窗格 id)。该名称在重启或部署交接后仍然保持,但它是一个名称:重命名、清除或复用它会改变 `gram list` 中显示为“我的”的项。认领(grab)本身始终是先到先得且原子的,与身份无关——两个智能体永远无法认领同一项。归属信息(`from` 以及所有者发布的消息)仅供参考,并非经过认证。

`send --file` 附加一个文件(说明文字可选),并分块上传以适应请求大小上限;`get-file` 用于下载。`delete` 会彻底删除消息及其附件字节——适合只发送一次随后即弃的短期机密(例如临时 API 密钥)。你可以删除自己发送的、认领的或发给你的消息;`--owner` 可删除任意消息。
...
使用 `--wait` 时，最初处于 settled 状态的智能体若生命周期序列前进，会返回 `delivery: "submitted"`。Herdr 会给 PTY 批次附加不透明的 composer attempt id。同一个 attempt 留在由光标佐证的编辑器中时返回 `agent_prompt_unsubmitted`，被观察到后清空时返回 `submitted`。bracketed paste 不依赖渲染后的 token 文本。

写入后仍无法证明去向时返回 `agent_prompt_stalled`。如果智能体已经处于 working，正在进行的其他轮次结束不会归因于新提示。`pane get` 会返回 `composer.state`（`empty`、`draft_present` 或 `unknown`）以及 provenance 和佐证。区域缺失、重绘中的画面、冲突和旧 daemon 响应都是 `unknown`。
