...
这些命令还会独立于生命周期状态公开进行中的模态输入请求。模态存在期间它们会包含 `"input_pending": true`,以及取值为 `confirm`、`select`、`free_text` 或 `unknown` 的 `input_prompt_kind`;其他情况下这两个字段都被省略。该信号不会改变 `agent_status`。
