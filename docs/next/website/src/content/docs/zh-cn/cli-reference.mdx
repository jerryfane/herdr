...
`agent prompt` 遵循当前的 bracketed paste 模式，即使智能体处于 working 也会原子地提交文本和编码后的 Enter。使用 `--wait` 时，最初处于 settled 状态的智能体若生命周期序列前进，会返回 `delivery: "submitted"`。规范化后的完整提示先出现、随后不再匹配，也会返回 `submitted`。对于 Claude 的 `[Pasted text #N]` 这类由清单声明的 bracketed-paste 标记，写入前的快照必须成功读出且没有既存标记；投递后标记必须出现，随后编辑器必须变为空。

完整提示或可归因的 paste 标记持续存在时返回 `agent_prompt_unsubmitted`。无法读取的基线、既存标记，或被另一段非空文本取代的标记都不能证明投递；没有其他证据时返回 `agent_prompt_stalled`。如果智能体已经处于 working，正在进行的其他轮次结束不会归因于新提示，必须有编辑器清除证据才能返回 `submitted`。同一窗格内并发投递的界面时序竞态明确推迟到 [#17](https://github.com/jerryfane/herdr/issues/17) 的写入侧来源证明。此后 Herdr 会等待请求的 settled 状态。`--until` 用于缩小匹配状态，不带 `--wait` 时会被拒绝。独立的 `agent wait` 在当前状态匹配时立即返回。两者默认匹配 `idle`、`done` 或 `blocked`；需要 `unknown` 时请明确使用 `--until unknown`。
