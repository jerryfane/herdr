...
如果智能体已经处于 `blocked`，`agent prompt --wait` 会在写入 PTY 前返回 `agent_blocked`。否则，Herdr 写入提示并检查是否有提交证据。只有确认提交后才等待指定状态；无法确认时，它返回 `type: "agent_prompted"`、`delivery: "written_to_pty"` 和退出码 0，而不是把已写入的提示当成发送失败。没有 composer 观测能力时，该回执可能很快返回；它不能证明智能体已开始新轮次。若同一个 attempt 的草稿仍留在 composer 中，则返回 `agent_prompt_unsubmitted`；PTY 写入前的失败仍是错误。独立的 `agent wait` 只观察当前状态，已匹配时立即返回。默认匹配 `idle`、`done` 或 `blocked`；`--until` 可以重复指定，而 `agent prompt` 使用它时必须带 `--wait`。

`delivery: "submitted"` 必须有可归因的证据：最初处于稳定状态的智能体生命周期序列前进，或者观察到同一个 writer-attributed composer attempt 随后清空。已经处于 working 的智能体完成别的轮次不能算作本次提示已提交。没有这种证据时，`written_to_pty` 只确认完整内容已写入终端。重试前先检查窗格，以免重复提交。观测错误和日志不会包含提示正文。

确认提交后，如果截止时仍未观察到请求的状态，则返回 `agent_status_unobserved_after_submit`：这是状态等待错误，不是投递失败。若直接进入请求的稳定状态，即使未观察到 `working` 也视为成功。不要因状态等待错误重发提示。
...
智能体已经处于 working 时，其他轮次的完成不会归因于新提示。只有确认提交后，`--wait` 才会继续等待请求的稳定状态；无法确认时返回 `written_to_pty` 回执，不继续等待该状态。如果只需观察状态而不推断是这条提示造成的变化，请另行使用 `agent wait`。两者默认匹配 `idle`、`done` 或 `blocked`。可重复使用 `--until idle --until done` 指定多个状态；需要 `unknown` 时请明确指定。在 `agent prompt` 中，`--until` 必须与 `--wait` 一起使用。
...
超时可能发生在 PTY 写入之前；`written_to_pty` 表示已写入，但无法确认智能体是否提交。重试前请检查窗格，避免重复发送。ID 和智能体名称只在所属服务器内有效；在 TUI 中切换机器不会改变已有窗格中 CLI 命令的目标。
