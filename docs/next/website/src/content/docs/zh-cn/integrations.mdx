...
OMP 集成通过 Herdr 的 socket API 上报智能体标签 `omp`、生命周期状态、规范化的原生会话路径、当前选中的会话树叶节点和准确的进程 PID。Herdr 可以在服务器重启后用 `omp --resume=<session>` 恢复 OMP 窗格,并使用叶节点和 PID 报告验证 OMP 会话迁移指向请求的分支和进程。
