...
`agent prompt --wait` submits immediately unless the agent is already `blocked`, in which case it returns `agent_blocked` without starting the wait. If an accepted prompt starts from another non-working state, Herdr must observe a lifecycle change within five seconds. Otherwise, rather than waiting indefinitely, it returns `agent_prompt_stalled` when the composer was observable and showed nothing, or `agent_prompt_unverifiable` when the pane exposes no composer observation at all; a caller `--timeout` of five seconds or less returns the normal `timeout` error. After Herdr observes activity, it waits for the requested settled status. It does not track individual turns. If the agent is already working, completion of that active turn may satisfy the wait. Standalone `agent wait` observes the current agent and returns immediately if its status already matches. Both commands default to `idle`, `done`, or `blocked`. Repeat `--until` to accept several exact states, for example `--until idle --until done`; use `--until unknown` explicitly when needed. On `agent prompt`, `--until` requires `--wait`.

Beyond the settled-status wait, `agent prompt --wait` distinguishes what Herdr can prove after the PTY acknowledgement. For an initially settled agent, a lifecycle-sequence advance reports `delivery: "submitted"`. Herdr also tags the PTY batch with an opaque attempt id and tracks composer input at the writer. If that attempt remains in a cursor-corroborated composer, the command returns `agent_prompt_unsubmitted`; if the same observed attempt clears, it reports `submitted`. This works for bracketed paste without inspecting or matching the hosted CLI's rendered paste token. If bytes were written but neither persistence nor attributable submission is observable, Herdr returns `agent_prompt_stalled`. That verdict now means Herdr **could** observe the composer and saw nothing either way. When the pane exposes no composer observation at all — the agent has no loadable manifest, or its manifest declares no `[composer]` section — Herdr returns `agent_prompt_unverifiable` instead. Neither verdict is proof of non-delivery, and a supervisor must not escalate on `agent_prompt_unverifiable` as though the prompt had been rejected: it reports a missing instrument, not a missing prompt.

For an already-working agent, an unrelated in-flight completion is not attributed to the new prompt: the same writer-attributed composer attempt must be observed and then clear for `submitted`, otherwise the success remains `written_to_pty`. Prompt text is not included in observation errors or logs.
...
After the delivery check, `--wait` waits for the requested settled status. Standalone `agent wait` observes the current agent and returns immediately if its status already matches. Both commands default to `idle`, `done`, or `blocked`. Repeat `--until` to accept several exact states, for example `--until idle --until done`; use `--until unknown` explicitly when needed. On `agent prompt`, `--until` requires `--wait`.
