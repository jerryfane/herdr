...
`agent prompt --wait` distinguishes what Herdr can prove after the PTY acknowledgement. For an initially settled agent, a lifecycle-sequence advance reports `delivery: "submitted"`; observing the exact normalized prompt fingerprint in the composer and then seeing it clear is stronger submission evidence. If the exact composer fingerprint persists, Herdr returns `agent_prompt_unsubmitted`. If bytes were written but neither persistence nor attributable submission is observable, it returns `agent_prompt_stalled`. For an already-working agent, an unrelated in-flight completion is not attributed to the new prompt: composer-observed-then-cleared reports `submitted`, otherwise the success remains `written_to_pty`. Prompt text is not included in observation errors or logs.
