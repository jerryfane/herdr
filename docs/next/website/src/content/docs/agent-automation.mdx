...
`agent prompt` submits text plus encoded Enter and honors the terminal's live bracketed-paste mode. It can prompt an agent that is already working. Herdr acknowledges only after the complete batch is written to the pane PTY. A non-wait success reports `delivery: "written_to_pty"`. If the agent is already `blocked`, it returns `agent_blocked` without sending terminal input; inspect the dialog and use `agent send-keys` for a deliberate response. If `input_pending` is already true, Herdr returns `agent_input_pending` with the prompt kind and writes zero bytes; use `agent send-keys` to interact deliberately with modal controls such as `esc`, `up`, or `enter`. `escape` is accepted as an alias for `esc`. Use the pane input commands when you deliberately want raw terminal control.
