...
`agent prompt --wait` distinguishes what Herdr can prove after the PTY acknowledgement. For an initially settled agent, a lifecycle-sequence advance reports `delivery: "submitted"`. Composer evidence can also prove delivery: the exact normalized prompt must appear and then stop matching, or a manifest-declared bracketed-paste token such as Claude's `[Pasted text #N]` must appear and then leave an empty composer. A paste token is attributable only when a successful pre-write snapshot showed no token already present. A persistent exact prompt or attributable paste token returns `agent_prompt_unsubmitted`; an unreadable baseline, a pre-existing token, or a token replaced by different non-empty composer text is not positive delivery evidence. If bytes were written but neither persistence nor attributable submission is observable, Herdr returns `agent_prompt_stalled`.

For an already-working agent, an unrelated in-flight completion is not attributed to the new prompt: composer-observed-then-cleared evidence is required for `submitted`, otherwise the success remains `written_to_pty`. Same-pane concurrent deliveries can still race because paste-token attribution is appearance-based; writer-side provenance in [#17](https://github.com/jerryfane/herdr/issues/17) will replace this mechanism. Prompt text is not included in observation errors or logs.
