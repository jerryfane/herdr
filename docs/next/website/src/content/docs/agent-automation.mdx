...
`agent get` and `agent list` expose modal input separately from lifecycle status:

```json
{
  "agent_status": "working",
  "input_pending": true,
  "input_prompt_kind": "select"
}
```

`input_prompt_kind` is `confirm`, `select`, `free_text`, or `unknown`. The fields are omitted when no input is pending, and `agent_status` remains unchanged.
...
`agent prompt` submits text plus encoded Enter and honors the terminal's live bracketed-paste mode. Herdr acknowledges only after the complete batch is written to the pane PTY. A non-wait success reports `delivery: "written_to_pty"`. If `input_pending` is already true, Herdr returns `agent_input_pending` with the prompt kind and writes zero bytes; use `agent send-keys` to interact deliberately with modal controls such as `esc`, `up`, or `enter`. `escape` is accepted as an alias for `esc`. Use the pane input commands when you deliberately want raw terminal control.
...
`agent prompt --wait` writes the prompt unless the agent is already `blocked`, which returns `agent_blocked` before any write. After an acknowledged PTY write, Herdr checks for submission evidence. If submission is confirmed, it waits for the requested agent status; if submission cannot be verified, it returns `type: "agent_prompted"` with `delivery: "written_to_pty"` and exit code 0 instead of treating the send as a failure. A manifest without composer verification can return this receipt promptly. This receipt does not prove the agent began a turn. A prompt still visible in its own attributed composer returns `agent_prompt_unsubmitted`; a failure before the PTY write remains an error.

`delivery: "submitted"` requires attributable evidence: a lifecycle sequence advance from an initially settled agent, or the same writer-attributed composer attempt being observed and then cleared. An unrelated in-flight completion on an already-working agent is not proof. Without that evidence, `written_to_pty` confirms only that the complete batch reached the terminal. Check the pane before resending to avoid a duplicate prompt. Prompt text is not included in observation errors or logs.

`pane get` includes a `composer` object. Its `state` is `empty`, `draft_present`, or `unknown`; `attempt_id` and the `evidence` fields explain the provenance and visual corroboration. Hosted-CLI suggestion text strictly right of the cursor is not a draft because Herdr did not write it. A missing composer region, a mid-repaint frame, conflicting signals, or a response from an older daemon that lacks `composer` is `unknown`, never silently `empty`. Cursor position only corroborates writer provenance. Style can only downgrade a result to `unknown`; colour alone never establishes `empty` or `draft_present`.

After verified submission, `--wait` waits for the requested settled status. If submission cannot be verified, it returns the PTY receipt without waiting for that status; use a separate `agent wait` if you need to observe it without claiming this prompt caused the transition. Standalone `agent wait` returns immediately if the current agent's status already matches. Both commands default to `idle`, `done`, or `blocked`. Repeat `--until` to accept several exact states, for example `--until idle --until done`; use `--until unknown` explicitly when needed. On `agent prompt`, `--until` requires `--wait`.

If submission is confirmed but the requested status is not observed by the deadline, `--wait` returns `agent_status_unobserved_after_submit` as a status-wait error, not a delivery failure. A fast turn that advances directly to a requested settled state counts without an intermediate working sample. Do not resend on a status-wait error.
...
A timeout can occur before the PTY write; `written_to_pty` means the write happened but submission was not confirmed. Check the agent before retrying to avoid submitting the same prompt twice. IDs and agent names are scoped to one server; selecting another machine in the TUI does not retarget CLI commands running in an existing pane.
...
Successful `agent start`, `agent prompt`, and `agent wait` commands return the current agent at `.result.agent`. `agent prompt` additionally reports `.result.delivery` as `written_to_pty` or `submitted`. `pane wait-output` returns `.result.pane_id`, `.result.matched_line`, and the matched snapshot at `.result.read`.
