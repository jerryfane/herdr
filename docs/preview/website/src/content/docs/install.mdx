...
A running server can also refuse a live handoff. Handoff works by starting a replacement and then letting the old process exit, so Herdr declines it when that exit would take the replacement down too — most importantly when Herdr is the main process of a systemd unit, where the main process exiting deactivates the unit and kills every pane in its control group. The request fails with an explicit error and nothing is torn down.

Deploy to a supervised install by replacing the binary and restarting the service instead. Note that restarting the service alone does not pick up a staged build: the swap that a handoff would have performed never happens, so the unit re-runs the same file. Move the staged binary over the live path first — a rename rather than a copy, since the running image is busy — and then restart the service.
