...
`submit-verify` 列（`--json` では `submission_verification_supported`）は、現在有効なマニフェストに `[composer]` があるかを示します。個々のペインで実際に観測できることまでは保証しません。
...
herdr gram list [--queue] [--unread] [--owner] [--limit N]  # --owner はオーナーとして読む、--limit は最新 N 件のみ
...
`report-agent-session` はライフサイクル状態を報告せずにネイティブセッション識別子を更新します。ツリー形式のセッションログを使うインテグレーションは、`--agent-session-cursor` で選択中のリーフを、`--agent-process-pid` でその報告に対応する正確なハーネスプロセスを指定できます。`release-agent` は、そのエージェントプロセスが終了したときに、そのソースのライフサイクル権限を終了します。
...
`agent prompt` は現在の bracketed paste モードを尊重し、working 中でもテキストを送信してから短い遅延の後にエンコード済み Enter を送信します。エージェントがすでに `blocked` の場合は、入力を送信せずに `agent_blocked` を返します。`--wait` を使う場合、別の non-working 状態から受け付けたプロンプトは 5 秒以内に観測可能なライフサイクル変化を起こす必要があり、変化がなければ Herdr は `agent_prompt_stalled`（コンポーザーを観測できて何も見えなかった場合）または `agent_prompt_unverifiable`（観測するコンポーザーがない場合）を返します。呼び出し側のタイムアウトが 5 秒以下なら、通常の `timeout` エラーを維持します。活動を観測した後、要求された安定状態を待ちます。個々のターンは追跡しません。すでに working の場合、進行中ターンの完了が待機を満たすことがあります。`--until` は一致状態を絞り込み、`--wait` なしでは拒否されます。単独の `agent wait` は現在の状態が一致すれば即座に返ります。どちらもデフォルトは `idle`、`done`、`blocked` です。

書き込まれたものの行方を証明できなければ `agent_prompt_stalled` です。すでに working の場合、進行中の別ターンの完了は新しいプロンプトに帰属されません。`pane get` の `composer.state` は `empty`、`draft_present`、`unknown` のいずれかで、provenance と補強証拠も返します。領域欠落、再描画途中、矛盾、古い daemon の応答は `unknown` です。その後 Herdr は要求された settled 状態を待ちます。`--until` は一致状態を絞り込み、`--wait` なしでは拒否されます。単独の `agent wait` は現在の状態が一致すれば即座に返ります。どちらもデフォルトは `idle`、`done`、`blocked` です。

書き込まれたものの行方を証明できなければ `agent_prompt_stalled` です。すでに working の場合、進行中の別ターンの完了は新しいプロンプトに帰属されません。`pane get` の `composer.state` は `empty`、`draft_present`、`unknown` のいずれかで、provenance と補強証拠も返します。領域欠落、再描画途中、矛盾、古い daemon の応答は `unknown` です。コンポーザー観測をまったく公開しないペインでは、`agent_prompt_stalled` ではなく `agent_prompt_unverifiable` を返します。呼び出し側は「観測したが何もなかった」と「観測できなかった」を区別できます。どちらも未送信の証明ではありません。
`agent prompt --wait` では、PTY への書き込み後に送信を確認できない 3 種類の結果を、`result.type: "agent_prompt_unconfirmed"`、`delivery: "written_to_pty"` と理由コード（`agent_prompt_unverifiable`、`agent_prompt_stalled`、`agent_prompt_unsubmitted`）として stdout に出力し、終了コード 0 で返します。再送前にペインを確認してください。有効なマニフェストに composer がない場合は、そのエージェントでは送信確認がサポートされていないことを明示します。直接 API を使う呼び出し元には従来のエラーコードが返ります。
...
`pane wait-output` と `agent wait` は、`--timeout` を省略すると無期限に待ちます。`agent prompt --wait` では、活動を観測した後、またはプロンプト開始時に `working` だった場合の安定状態待機は無期限です。non-working 状態から開始して活動を観測できない場合は、5 秒後に送信未確認の結果を返します。タイムアウトは PTY への書き込み確認前にも起こり得るため、引き続きエラーです。実際の配信失敗やサーバーエラーは JSON として stderr に出力され、終了コード 1 になります。CLI 使用法のエラーは終了コード 2 です。
