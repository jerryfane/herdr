...
`agent prompt --wait` は、エージェントがすでに `blocked` でない限り、プロンプトをすぐ送信します。すでに `blocked` の場合は待機を開始せずに `agent_blocked` を返します。受け付けたプロンプトが別の non-working 状態から始まる場合、Herdr はまず 5 秒以内にライフサイクル変化が観測されることを要求します。状態シーケンスが進まなければ、無期限に待たず、コンポーザーを観測できて何も見えなかった場合は `agent_prompt_stalled`、コンポーザー観測をまったく公開していないペインの場合は `agent_prompt_unverifiable` を返します。呼び出し側の `--timeout` が 5 秒以下なら、通常の `timeout` エラーを返します。活動を観測した後、要求された安定状態を待ちます。個々のターンは追跡しません。エージェントがすでに working の場合、進行中ターンの完了が待機を満たすことがあります。単独の `agent wait` は現在のエージェントを監視し、すでに状態が一致していれば即座に返ります。どちらもデフォルトでは `idle`、`done`、`blocked` を待ちます。複数の正確な状態を許可するには、`--until idle --until done` のように `--until` を繰り返します。`unknown` が必要な場合は `--until unknown` を明示してください。`agent prompt` では `--until` に `--wait` が必要です。

`agent prompt --wait` は PTY の確認応答後に Herdr が証明できる配送状態を区別します。最初に settled だったエージェントでは、ライフサイクルシーケンスが進むと `delivery: "submitted"` になります。Herdr は PTY バッチに不透明な attempt id も付け、writer でコンポーザー入力を追跡します。同じ attempt がカーソルで裏付けられたコンポーザーに残れば `agent_prompt_unsubmitted`、観測後に消えれば `submitted` です。bracketed paste も表示された paste token の文字列には依存しません。それ以外で書き込み後の行方を観測できなければ `agent_prompt_stalled` になります。 この判定は、Herdr がコンポーザーを**観測できた**うえで、どちらの証拠も得られなかった場合を意味します。ペインがコンポーザー観測をまったく公開していない場合（エージェントのマニフェストに `[composer]` 領域が宣言されていない、またはマニフェストに `[composer]` セクションがない場合）、Herdr は代わりに `agent_prompt_unverifiable` を返します。どちらの判定も未送信の証明ではありません。`agent_prompt_unverifiable` は計器の欠如を報告するものであり、プロンプトが拒否されたかのようにエスカレーションしてはいけません。
...
エージェントがすでに working の場合、進行中の別ターンの完了は新しいプロンプトに帰属されません。`submitted` には同じ writer-attributed attempt が観測され、その後クリアされた証拠が必要で、それ以外の成功は `written_to_pty` のままです。配送確認の後、Herdr は要求された settled 状態を待ちます。単独の `agent wait` は現在のエージェントを監視し、すでに状態が一致していれば即座に返ります。どちらもデフォルトでは `idle`、`done`、`blocked` を待ちます。複数の正確な状態を許可するには、`--until idle --until done` のように `--until` を繰り返します。`unknown` が必要な場合は `--until unknown` を明示してください。`agent prompt` では `--until` に `--wait` が必要です。
