...
运行中的服务器也可能拒绝实时移交。移交的做法是先启动一个替代进程,再让旧进程退出;如果这次退出会把替代进程一起带走,Herdr 就会拒绝移交。最典型的情况是 Herdr 是某个 systemd 单元的主进程:主进程退出会停用该单元,进而杀掉其控制组中的每一个窗格。此时请求会以明确的错误失败,不会拆除任何东西。

要向受服务管理的安装发布,请改为替换二进制文件后重启服务。注意仅重启服务并不会启用已暂存的构建:移交本应完成的替换没有发生,单元只会重新运行同一个文件。请先把暂存的二进制文件移动到运行路径上(用 rename 而不是复制,因为运行中的映像正被占用),然后再重启服务。
