...
| 带 `--handoff` 的更新 | 对受支持的运行中服务器尽力而为;在 systemd 等服务管理器之下会被直接拒绝 | 是 | 是,交接成功时来自实时终端 | 是,交接成功时进程保持运行 |
...
选择启用并不保证交接一定发生。如果服务器自身的退出会摧毁替代进程,它就会拒绝交接——最典型的是 Herdr 作为 systemd 单元的主进程时:`Type=simple` 单元在主进程退出时被停用,默认的 `KillMode=control-group` 随即杀掉替代进程以及所有窗格。拒绝发生在拆除任何东西之前,因此现有窗格继续运行,请求返回错误而不是一次残缺的交接。受服务管理的安装请通过替换二进制文件并重启服务来更新,见[安装 Herdr](/zh-cn/docs/install/#update)。
