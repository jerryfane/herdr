...
`submit-verify` 列（`--json` 中为 `submission_verification_supported`）表示当前生效的清单是否声明了 `[composer]`，但不保证某个窗格在运行时一定可观察。
...
herdr gram list [--queue] [--unread] [--owner] [--limit N]  # --owner 以所有者身份查看;--limit 仅取最新 N 条
...
`report-agent-session` 只更新原生会话身份,不上报生命周期状态。使用树形会话日志的集成还可以用 `--agent-session-cursor` 上报当前选中的叶子,并用 `--agent-process-pid` 将该报告绑定到准确的运行时进程。`release-agent` 在智能体进程退出时结束该来源的生命周期权威。
...
`agent prompt` 遵循当前的 bracketed paste 模式,即使智能体处于 working 也会先发送文本,短暂延迟后再发送编码后的 Enter。如果智能体已经是 `blocked`,它不会发送输入,而是返回 `agent_blocked`。使用 `--wait` 时,从其他非 working 状态接受的提示必须在五秒内产生可观察的生命周期变化，否则 Herdr 返回 `agent_prompt_stalled`（能观察到 composer 但没有看到证据）或 `agent_prompt_unverifiable`（没有可观察的 composer）；调用方超时不超过五秒时，仍返回普通的 `timeout` 错误。观察到活动后，它会等待请求的稳定状态。它不会跟踪单独的轮次。如果智能体已经处于 working，当前轮次的完成可能满足等待。`--until` 用于缩小匹配状态，不带 `--wait` 时会被拒绝。独立的 `agent wait` 在当前状态匹配时立即返回。两者默认匹配 `idle`、`done` 或 `blocked`；需要 `unknown` 时请明确使用 `--until unknown`。

写入后仍无法证明去向时返回 `agent_prompt_stalled`。如果智能体已经处于 working，正在进行的其他轮次结束不会归因于新提示。`pane get` 会返回 `composer.state`（`empty`、`draft_present` 或 `unknown`）以及 provenance 和佐证。区域缺失、重绘中的画面、冲突和旧 daemon 响应都是 `unknown`。此后 Herdr 会等待请求的 settled 状态。`--until` 用于缩小匹配状态，不带 `--wait` 时会被拒绝。独立的 `agent wait` 在当前状态匹配时立即返回。两者默认匹配 `idle`、`done` 或 `blocked`；需要 `unknown` 时请明确使用 `--until unknown`。

写入后仍无法证明去向时返回 `agent_prompt_stalled`。如果智能体已经处于 working，正在进行的其他轮次结束不会归因于新提示。`pane get` 会返回 `composer.state`（`empty`、`draft_present` 或 `unknown`）以及 provenance 和佐证。区域缺失、重绘中的画面、冲突和旧 daemon 响应都是 `unknown`。对于完全不暴露 composer 观察的窗格，返回 `agent_prompt_unverifiable` 而非 `agent_prompt_stalled`，以便调用方区分“已观察但无证据”与“无法观察”；两者都不能证明未送达。
对于 `agent prompt --wait`，PTY 已写入但无法确认提交的三种结果会写到 stdout：`result.type: "agent_prompt_unconfirmed"`、`delivery: "written_to_pty"`，以及 `agent_prompt_unverifiable`、`agent_prompt_stalled` 或 `agent_prompt_unsubmitted` 作为原因代码。CLI 退出状态为 0；再次发送前先检查窗格。若当前清单没有 composer 覆盖，提示会明确说明该智能体不支持提交验证。直接调用 API 仍会收到原有错误代码。
...
`pane wait-output` 和 `agent wait` 在省略 `--timeout` 时会无限等待。对于 `agent prompt --wait`，观察到活动后，或提示开始时已处于 `working`，等待稳定状态的过程不会超时；如果提示从 non-working 状态开始且未观察到活动，五秒后会返回提交未确认的结果。超时可能发生在 PTY 写入得到确认之前，仍属于错误。真正的投递失败和服务器错误以 JSON 写到 stderr，退出状态为 1；CLI 用法错误的退出状态为 2。
