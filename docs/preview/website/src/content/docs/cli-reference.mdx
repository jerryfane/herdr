...
`pane wait-output` and `agent wait` wait indefinitely when `--timeout` is omitted. For `agent prompt --wait`, the settled-state wait is indefinite after activity is observed or when the prompt starts in `working`; a non-working prompt still reports an unconfirmed result after five seconds without observed activity. A timeout may occur before PTY write confirmation and remains an error. Genuine delivery and server errors appear as JSON on stderr with exit status 1; CLI usage errors exit with status 2.
