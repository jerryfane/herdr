...
| Update with `--handoff` | Best effort for supported running servers; refused outright under a service manager such as systemd | Yes | Yes, from the live terminal if handoff succeeds | Yes, because the process keeps running if handoff succeeds |
...
Opting in does not guarantee a handoff. The running server refuses one when its own exit would destroy the replacement — notably when Herdr is the main process of a systemd unit, because a `Type=simple` unit deactivates when its main process exits and the default `KillMode=control-group` then kills the replacement and every pane with it. The refusal happens before anything is torn down, so the existing panes keep running and the request returns an error rather than a partial handoff. Update supervised installs by replacing the binary and restarting the service; see [Install Herdr](/docs/install/#update).
