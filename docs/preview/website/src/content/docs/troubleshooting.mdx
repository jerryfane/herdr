...
If Herdr runs under a service manager such as a systemd unit, restarting the service is not enough on its own: the unit re-runs the same binary path, so it starts the old build again. Install the new binary over that path first, then restart the service.
