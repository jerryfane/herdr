...
The OMP integration reports `omp` as the agent label, lifecycle state, canonical native session path, selected conversation-tree leaf, and exact process PID through Herdr's socket API. It does not require native process detection for the `omp` executable, and Herdr can resume an OMP pane with `omp --resume=<session>` after a server restart. Herdr uses the leaf and PID reports to prove OMP session transfers target the requested branch and process.
