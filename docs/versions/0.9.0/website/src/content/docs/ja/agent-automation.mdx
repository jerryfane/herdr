...
`agent get` と `agent list` は、モーダル入力をライフサイクル状態とは別に公開します:
...
`input_prompt_kind` は `confirm`、`select`、`free_text`、`unknown` のいずれかです。入力待ちがないときは両方のフィールドが省略され、`agent_status` は変わりません。
