...
フェデレーションを使うと、ある Herdr デーモンから別のデーモンのランタイムにアクセスできます。デフォルトでは無効です。受信 TCP リスナーがバインドされるのは、フェデレーションを有効にし、かつ認証に使うピアトークンを 1 つ以上指定した場合だけです。そうでない場合は、認証のないポートを公開する代わりに起動を拒否します。
...
`[[federation.peers]]` テーブルは、それぞれ 1 つのピアを宣言します:

- `alias` — ログや診断で使われる、そのピアのローカル名。
- `token_file` — 共有シークレットのパス。前後の空白を除いた内容が、このピアが提示するトークンになります。ファイルのバイト列がログに記録されることはありません。読み取り可能で空でないトークンファイルを持たないピアはスキップされます。
- `endpoint` — 送信接続でそのピアに到達するための接続先（今後のフェデレーション機能で使用）。
- `expected_node_id` — 接続時に検証するノード ID（今後のフェデレーション機能で使用）。
- `capability` — このピアのトークンが受信 TCP フェデレーションで得る権限レベル。`observe`、`interact`、`admin` のいずれか。デフォルト: `observe`。

権限レベルには順序があり、上位のレベルは下位のレベルをすべて含みます:

- `observe` — 読み取り専用。ペイン、エージェント、イベント、レイアウトの一覧取得、読み取り、確認ができます。
- `interact` — `observe` のすべてに加えて、エージェントとペインの操作（`agent.prompt`、`agent.send_keys`、`pane.send_text`、`pane.send_keys`）。
- `admin` — `interact` のすべてに加えて、フォーカス、リネーム、入力と権限の変更（`agent.focus`、`agent.rename`、`pane.rename`、`pane.input.set`、`pane.send_input`、`agent.view.set`、`agent.view.clear`）。

フェデレーションが公開するのは、意図的に選ばれた「読み取りと操作」だけの API サブセットです。どの権限レベルにも含まれないもの、つまりサーバーのライフサイクル、プラグイン、インテグレーション、gram、通知、ウィンドウタイトル、エージェント / ペイン / ワークスペース / タブ / worktree / レイアウトの変更、グラフィックスは、権限レベルにかかわらずすべてのフェデレーションピアに対して拒否されます。受信接続にも上限があるため、ピアがデーモンのスレッドやファイルディスクリプタを使い果たすことはできません。

:::caution[セキュリティ]
TCP フェデレーションのトークンが与えるのは、そのピアの権限レベルちょうどであり、それ以上ではありません。各トークンファイルは、その権限レベルに限定されたシークレットとして扱ってください。監視だけが必要なピアには `observe` のトークンを渡し、`admin` は完全に信頼できるピアのために取っておきます。これは SSH 経路（`herdr --remote` / `herdr api-bridge`）とは異なります。SSH 経路はローカルの unix ソケットに対して動作するため、フィルタされない完全なコントロールプレーンアクセスを与えます。フェデレーションの権限レベルは SSH 経路には適用されません。ローカルの unix ソケット自体は、トークンの検証も権限レベルによるフィルタリングも一切行いません。
:::
